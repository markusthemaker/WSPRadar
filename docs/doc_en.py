...
Select one station to open the selected station evidence view. Selecting another row replaces the current station, while clearing the row hides the view. `Station Insights` continues to list the complete `callsign + locator` population contributing to the selected segment; Compare rows show joint and exclusive evidence plus station-level median Delta SNR, and `Include Unpaired Evidence` includes identities represented only by exclusive or asynchronous evidence. Below the station table, the Performance-style prompt `↓ Select time aggregation bin size:` controls the left **Δ SNR over Time** panel, which preserves the selected station's actual UTC sequence at the chosen bin. The right **Δ SNR by UTC Hour** panel is shown simultaneously and folds the same Joint Spots or Scheduled Pairs from all represented dates into fixed one-hour UTC slots. Both panels use the median-centered nonlinear Delta SNR scale of that station's selected evidence, not the segment median above. UTC-hour folding requires evidence from at least two distinct UTC dates; otherwise the folded panel is omitted, the chronological panel expands and the unavailable-view notice remains visible.
...
| **Reference-side SNR correction (dB)** | `0.0 dB`; range `-99.9` to `+99.9 dB` | Added to the Reference-side SNR before Delta SNR is calculated. Guided displays the number for `Use an established correction`; Classic exposes it for every Compare design. A nonzero Classic edit selects `established_offset`, while `0.0 dB` preserves the current explicit mode. No-offset and offset-establishment runs keep the value at `0.0 dB`; an established correction may be signed or explicitly established as `0.0 dB`. |
...
The transform does not change scientific values or grouping. Segment-histogram counts and bin edges remain in raw dB, temporal cells remain rounded integer-dB bins, medians remain raw-dB statistics, and relative-density colors retain the calculation above. Because nonlinear vertical stretching gives equal raw-dB bins in the retained Segment Compare histograms unequal displayed heights, read histogram **bar length** against `Share (%)`; displayed bar area is not probability. Performance SNR figures remain linear.

Selected Station Evidence in Compare filters the retained active-scope rows to exactly one selected `callsign + locator` identity. Its single full-width figure reuses the temporal layout and shared title, subtitle, colorbar and spacing primitives used by Performance. **Δ SNR over Time** places every retained Joint Spot or Scheduled Pair from that path in its actual UTC sequence using the selected aggregation bin. **Δ SNR by UTC Hour** folds those same observation-level rows across all represented dates into fixed one-hour UTC slots; it does not first reduce or equally weight dates. Each panel normalizes density independently to its own most populated cell, while the two panels share the selected path's median-centered nonlinear Delta SNR axis and absolute dB labels. UTC-hour folding requires at least two represented dates; with fewer dates, the folded panel is omitted, the chronological panel expands and the localized unavailable notice is retained. Changing the selected bin changes only this retained-evidence presentation and never changes pairing or reruns the provider analysis.
