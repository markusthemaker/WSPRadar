...
* [4. Methods for Comparative Analysis](#sec-4)
  * [Option 1: Local Neighborhood Benchmark](#sec-4-1)
...
<a id="sec-4-1"></a>
#### Option 1: Local Neighborhood Benchmark
* **Objective:** Measure your capability against active WSPR stations inside your defined neighborhood radius.
* **Methodology:** WSPRadar collects local WSPR stations within the selected radius (max 250 km) and applies strict *Cycle-by-Cycle (Spot-by-Spot)* mathematics against the same remote station in the same 2-minute WSPR cycle. This mode now has two selectable reference definitions:
  * **Local Best Station:** For every cycle and remote path, WSPRadar compares you against the strongest active local station in the radius. The benchmark asks: *"If the best station in my direct neighborhood reached/heard this DX station, how did I perform in direct comparison?"* This is the strictest local benchmark.
  * **Local Median Neighborhood:** For every cycle and remote path, WSPRadar computes the median normalized SNR of all active local stations in the radius and compares you against that neighborhood median. This answers the more typical question: *"Am I above or below the central performance of my active local neighborhood?"*
...
* **Benchmark Mode:** Selects the fundamental type of comparison (Local Neighborhood Benchmark, Specific Reference Station, or Hardware A/B-Test).
...
* **Min. Spots/Station (Data Robustness):** To ensure high data robustness against noise and artifacts, this filter prevents random single decodes ("One-Hit Wonders") from skewing the results. The filter philosophy adapts automatically based on your selected comparison mode:
  ...
  * **Local Neighborhood Benchmark:** When comparing against a geographic radius, the reference is either the strongest active local station (`Local Best Station`) or the median of all active local stations (`Local Median Neighborhood`). The filter evaluates the qualifying reference data for the same remote peer and cycle, without treating missing WSPR spots as numeric SNR values.
