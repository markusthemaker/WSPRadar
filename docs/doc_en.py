...
The four Compare distribution and temporal panels use a presentation-only, median-centered nonlinear Delta SNR scale. The two segment temporal panels share the observation-level median of all paired evidence in the selected segment. The selected-station distribution and its active time panel instead share the median of the one selected station's evidence. Thus each two-panel evidence scope has one center, while absolute labels preserve interpretation between scopes.
...
* Save the versioned `.config`. It records the settings applicable to the run:
    ...
    * **durable result-view settings:** selected ranges and directions, the selected station, evidence time bins and temporal view, and visibility of unpaired evidence or counter-only stations in Performance.
