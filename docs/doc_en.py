...
* **The TX A/B Test (Sequential / Time-Slicing):** You use two identical WSPR transmitters or a transceiver that periodically switches between transmitting to Antenna A and Antenna B based on timing. Since Setup A and B *never* transmit in the same slot, WSPRadar uses **Asynchronous Math (Time-Averaging)**: It splits your data based on the start minutes (Even: 00, 04, 08 / Odd: 02, 06, 10) and forms long-term medians before calculating the $\Delta$ SNR. *(Because Setup A and B never transmit in the same slot, there are physically no simultaneous 'Joint Spots'. Therefore, the Segment Inspector utilizes the 'Async Both' logic to evaluate intersecting stations.)*
...
3. **Station Insights Table:** Below the histogram, an interactive data table lists the exact remote stations involved in the selected segment. By default, for clarity, only stations with successful `Joint Spots` are listed (exception: sequential TX A/B tests dynamically default to showing all raw spots). The "Median Δ SNR" column shows the merged value for exactly this one station. In comparative modes, the table also details how many joint decodes (`Joint Spots`) form this value and how many exclusive decodes (`Only Ref` or `Only [Callsign]`) exist.
