...
Wenn sich die Ergebnisse öffnen, folge dem oben eingeführten Evidenzpfad. Die Bereiche Performance-Evidenz, Vergleichsevidenz, Zeitliche Evidenz und Evidenz ausgewählter Stationen bleiben an den jeweils zutreffenden Stellen dieses Ablaufs verfügbar.
...
* RX Performance zeigt `Vom Target gehört`, `Nur von anderen gehört` und `Vom Target gehört, aber nicht unabhängig bestätigt`.
* TX Performance zeigt `Target gehört`, `Nur andere Signale gehört` und `Target gehört, RX-Aktivität nicht unabhängig bestätigt`.
...
- `Stationen „Nur von anderen gehört“ anzeigen` bei RX beziehungsweise `Stationen „Nur andere Signale gehört“ anzeigen` bei TX stellt qualifizierende Performance-Identitäten mit ausschließlicher Gegen-Evidenz wieder dar. Die Einstellung wird für Performance gespeichert.
