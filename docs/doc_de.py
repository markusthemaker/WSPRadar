...
Wähle eine Station aus, um ihre Evidenzansicht zu öffnen. Die Auswahl einer anderen Zeile ersetzt die bisherige Station, während das Aufheben der Auswahl die Ansicht ausblendet. `Station Insights` listet weiterhin die vollständige Population der Identitäten aus `callsign + locator` auf, die zum ausgewählten Segment beitragen. Compare-Zeilen zeigen Joint- und exklusive Evidenz sowie das stationsbezogene mediane Delta SNR; `Ungepaarte Evidenz einbeziehen` schließt Identitäten ein, die ausschließlich durch exklusive oder asynchrone Evidenz vertreten sind. Unter der Stationstabelle steuert die wie in Performance gestaltete Aufforderung `↓ Zeitliche Aggregationsbreite auswählen:` das linke Panel **Δ SNR im Zeitverlauf**, das die tatsächliche UTC-Folge der ausgewählten Station mit dem gewählten Bin erhält. Das rechte Panel **Δ SNR nach UTC-Stunde** erscheint gleichzeitig und faltet dieselben Joint Spots oder Scheduled Pairs aus allen berücksichtigten Tagen in feste einstündige UTC-Slots. Beide Panels verwenden die medianzentrierte, nichtlineare Delta-SNR-Skala der ausgewählten Evidenz dieser Station, nicht den darüber angezeigten Segmentmedian. Die Faltung nach UTC-Stunde erfordert Evidenz aus mindestens zwei verschiedenen UTC-Tagen; andernfalls entfällt das gefaltete Panel, das chronologische Panel wird erweitert und der Hinweis zur nicht verfügbaren Ansicht bleibt sichtbar.
...
Die Transformation verändert weder wissenschaftliche Werte noch Gruppierungen. Anzahlen und Klassengrenzen der Segment-Histogramme bleiben in untransformierten dB-Werten, die Zeitzellen bleiben auf ganzzahlige dB gerundete Klassen, Mediane bleiben Statistiken aus untransformierten dB-Werten, und die Farben der relativen Dichte behalten die oben angegebene Berechnung bei. Weil die nichtlineare vertikale Streckung gleich breiten Roh-dB-Klassen in den beibehaltenen Histogrammen von Segment Compare unterschiedliche dargestellte Höhen gibt, ist die **Balkenlänge** auf der Achse `Anteil (%)` abzulesen; die dargestellte Balkenfläche ist keine Wahrscheinlichkeit. Performance-SNR-Diagramme bleiben linear.

Die Evidenz der ausgewählten Station in Compare filtert die beibehaltenen Zeilen des aktiven Bereichs auf genau eine ausgewählte Identität aus `Rufzeichen + Locator`. Ihre einzelne vollbreite Abbildung verwendet erneut das zeitliche Layout sowie die gemeinsamen Titel-, Untertitel-, Farbleisten- und Abstandsbausteine von Performance. **Δ SNR im Zeitverlauf** ordnet jeden beibehaltenen Joint Spot beziehungsweise jedes Scheduled Pair dieses Funkwegs mit der gewählten Aggregationsbreite in seiner tatsächlichen UTC-Folge an. **Δ SNR nach UTC-Stunde** faltet dieselben Evidenzzeilen auf Beobachtungsebene über alle berücksichtigten Tage in feste einstündige UTC-Slots; die Tage werden vorher weder zusammengefasst noch gleich gewichtet. Jedes Panel normiert seine Dichte unabhängig auf seine am stärksten belegte Zelle, während beide Panels dieselbe medianzentrierte, nichtlineare Delta-SNR-Achse des ausgewählten Funkwegs mit absoluten dB-Beschriftungen verwenden. Die Faltung nach UTC-Stunde erfordert mindestens zwei berücksichtigte Tage; bei weniger Tagen entfällt das gefaltete Panel, das chronologische Panel wird erweitert und der lokalisierte Hinweis zur nicht verfügbaren Ansicht bleibt erhalten. Ein Wechsel des ausgewählten Bins verändert nur diese Darstellung aus beibehaltener Evidenz, weder Paarbildung noch abgeschlossene Provider-Abfrage.
