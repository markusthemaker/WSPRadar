...
`Station Insights` listet die Identitäten `callsign + locator` auf, die zum ausgewählten Segment beitragen. Success-Zeilen zeigen Target- und Elsewhere- beziehungsweise Other-Signals-Evidenz, Success Rate und den Median des erfolgreichen Target-SNR, normiert auf 30 dBm. Lies jede Rate zusammen mit ihren Anzahlen für Target und Gegen-Evidenz; mit `Zero-Target-Stationen zeigen` lassen sich qualifizierende Stationen ohne Target-Evidenz wieder einblenden.
...
Die gespeicherte Konfiguration enthält die zutreffenden ausführbaren Einstellungen. `run_metadata.json` erfasst automatisch Anwendungsname und -version, Exportzeit, Sprache, Richtung, Band, Benchmark-Auswahl, konfigurierte Zeitauswahl, Modus und Zahlenwert der referenzseitigen Korrektur als `benchmark_snr_correction_mode` und `benchmark_snr_correction_db`, Filter, Schwellen, Ergebnisblöcke und Auswahlen in der Inspektionsansicht.
