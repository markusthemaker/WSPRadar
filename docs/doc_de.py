...
Wenn sich die Ergebnisse öffnen, folge dem oben eingeführten Evidenzpfad. Die Bereiche Success-Evidenz, Vergleichsevidenz, Zeitliche Evidenz und Evidenz ausgewählter Stationen bleiben an den jeweils zutreffenden Stellen dieses Ablaufs verfügbar.
...
Es gibt weder eine Referenzstation noch einen Referenzpfad. Die Success Rate beschreibt bedingte Erreichbarkeit; der SNR erfolgreicher Target-Decodes ist eine getrennte Signalstärkezusammenfassung. [Abschnitt 2.1](#sec-3-2) erläutert die sichtbaren Klassifikationen und Gewichtungen, [Abschnitt 7.4](#sec-7-4) definiert den exakten Nenner.
...
* Prüfe ausgewählte Stationen und gehe anschließend im Drill-Down bis zur Evidenz auf Zeilenebene.
...
WSPRadar verwendet im sichtbaren Success-Ergebnis durchgehend ein richtungsspezifisches Klartextvokabular:
...
* **Nur prüfbare Evidenz:** RX zeigt <strong class="defined-term">Vom Target gehört, aber nicht unabhängig bestätigt</strong>; TX zeigt <strong class="defined-term">Target gehört, RX-Aktivität nicht unabhängig bestätigt</strong>. Diese Zeilen bleiben prüfbar, gehen aber nicht in die Success Rate ein.
...
Die Success Rate wird nicht auf die Sendeleistung normiert. Der daneben angezeigte SNR erfolgreicher Target-Decodes wird dagegen auf die gemeldete Leistung von 30 dBm normiert.

Angezeigte `100%` bedeuten, dass das Target bei jeder qualifizierenden Opportunity der Station oder des ausgewählten Bereichs erfolgreich war. Sie bedeuten nicht, dass jede mögliche oder geplante Aussendung decodiert wurde. Da Success mit einer anspruchsvollen, weltweit gebildeten Opportunity-Population beginnt und anschließend den konfigurierten geografischen Analyseumfang anwendet, ergibt sich die praktische Aussage aus Geografie, qualifizierenden Stationen, bestätigten Opportunities, Zeitverlauf und Wiederholung – nicht allein aus der Nähe zu `100%`.
...
* Success: Die Sektorfüllung ist die einzige quantitative Farbebene und zeigt die stationsgleichgewichtete Success Rate des Entfernungs- und Richtungssegments. Bei RX bedeutet ein kleiner dunkelgrüner Vollmarker `Vom Target gehört`, ein kleiner hellgrauer Vollmarker `Nur von anderen gehört`. Bei TX lauten die entsprechenden Bezeichnungen `Target gehört` und `Nur andere Signale gehört`. Alle sichtbaren Stationsmarker verwenden eine feste Größe und codieren weder die individuelle Success Rate noch die Evidenztiefe. Wenn Identitäten dieselben Kartenkoordinaten teilen, werden dunkelgrüne Marker über hellgrauen Markern gezeichnet; ein sichtbarer Ort kann dennoch mehrere Identitäten aus `callsign + locator` repräsentieren.
* Compare: Joint ist grün, Both (Async) gelborange, Only Target violett und Only Reference weiß.

Ein gültiges Success-Segment bei `0%` bleibt Teil der Success-Rate-Skala. `Unzureichende Evidenz` ist ein anderer Zustand: Das Segment erfüllt die konfigurierten Anforderungen an qualifizierende Evidenz nicht und bleibt ungefüllt, sodass die neutrale Basiskarte außerhalb der Success-Rate-Skala sichtbar ist. Unzureichende Evidenz darf nicht als gemessene Success Rate von `0%` gelesen werden.
...
* Auf Success-Karten beschreibt die obere Zeile <strong class="defined-term">GELEGENHEITEN</strong> die Nennertiefe als `Vom Target gehört` plus `Nur von anderen gehört` bei RX beziehungsweise `Target gehört` plus `Nur andere Signale gehört` bei TX. Die untere Zeile <strong class="defined-term">STATIONEN</strong> verwendet dieselben beiden richtungsspezifischen Bezeichnungen für die Breite über unterschiedliche qualifizierende Identitäten `callsign + locator`.
...
Bei RX Success verwenden beide Zeilen am Kartenfuß `Vom Target gehört` und `Nur von anderen gehört`; die Zeilenüberschrift unterscheidet einzelne bestätigte Gelegenheiten von qualifizierenden TX-Stationsidentitäten. Bei TX Success verwenden beide Zeilen `Target gehört` und `Nur andere Signale gehört` mit derselben Unterscheidung zwischen Gelegenheiten und Stationen. Exakte Anzahlen erscheinen in ausreichend breiten Segmenten; bei schmalen Segmenten folgt die Darstellung dem kompakten Compare-Kartenfuß und erzwingt keinen überlappenden Text. Evidenz ohne die für den Nenner erforderliche unabhängige Bestätigung und nicht qualifizierende Evidenz werden ausgeschlossen, weil sie nicht in die Success Rate eingehen.
...
Die Bereichszusammenfassung trennt die Breite der Stationsbasis von der Tiefe der bestätigten Gelegenheiten. Die Zeile `Stationen` zeigt die modusspezifischen Stationsgruppen—`Vom Target gehört` und `Nur von anderen gehört` bei RX, `Target gehört` und `Nur andere Signale gehört` bei TX—sowie eine `Success Rate`. Dabei handelt es sich um die stationsgleichgewichtete Success Rate: Jede qualifizierende Station erhält über ihre eigene Success Rate eine Stimme. Die Anzahlen der Stationsgruppen beschreiben eine Mindestens-einmal-Reichweite und bilden daher nicht den arithmetischen Zähler und Nenner dieser Rate. Die Zeile `Gelegenheiten` verwendet dieselben richtungsspezifischen Outcome-Bezeichnungen und zeigt die Success Rate auf Beobachtungsebene, bei der jede bestätigte Gelegenheit eine Stimme erhält. Ihre angezeigten Anzahlen bilden den Zähler und Nenner dieser Rate. Wenn sich der Evidenzumfang zwischen Stationen unterscheidet, können die beiden Zeilen deshalb unterschiedliche Success Rates zeigen.

Nach der Auswahl von Entfernungsbereichen und Richtungen berechnet WSPRadar alle folgenden Success-Ansichten aus der vollständigen qualifizierenden Stationspopulation dieses aktiven Bereichs. Filter, Sortierung, das richtungsspezifische Bedienelement `Stationen „Nur von anderen gehört“ anzeigen` beziehungsweise `Stationen „Nur andere Signale gehört“ anzeigen` und die Zeilenauswahl in Station Insights verändern diese Segmentansichten nicht.
...
Das mittlere Panel heißt **RX Success Rate nach Entfernung der TX-Station** beziehungsweise **TX Success Rate nach Entfernung der RX-Station**. Es fragt, wie zuverlässig das Target innerhalb aller bestätigten Gelegenheiten erfolgreich war. Die stationsgleichgewichtete Linie gibt jeder Station eine Stimme. Die Linie auf Beobachtungsebene gibt jeder Gelegenheit eine Stimme. Ihr Unterschied zeigt, ob Stationen mit viel Evidenz anders abschneiden als die breite Stationspopulation. Hohe Mindestens-einmal-Reichweite bei niedriger Success Rate bedeutet, dass viele Funkwege mindestens einmal offen, aber wechselhaft waren; niedrige Reichweite bei hoher Success Rate bedeutet, dass weniger Wege offen, diese aber vergleichsweise zuverlässig waren.

Das rechte Panel heißt bei RX **Erfolgreiches Target-SNR nach Entfernung der TX-Station** und bei TX **Erfolgreiches Target-SNR nach Entfernung der RX-Station**. Es beschreibt ausschließlich erfolgreiche Target-Outcomes. WSPRadar bildet zunächst je Station einen Median des erfolgreichen SNR und fasst danach diese Stationsmediane in denselben exakten Entfernungs-Bins mit Median und Streuung zusammen. Dadurch dominiert eine häufig meldende Station das Entfernungsprofil nicht. Für ein nicht decodiertes Signal gibt es kein Target-SNR; es kann in diesem Panel nicht erscheinen. Lies dies zusammen mit der Success Rate: Steigt das erfolgreiche SNR bei gleichzeitig fallender Success Rate, können schwächere Signale unter die Decode-Schwelle verschwunden sein, sodass nur stärkere erfolgreiche Decodes übrig bleiben.
...
**Zeitliche Evidenz** trennt denselben aktiven geografischen Bereich in zwei vertikal ausgerichtete Abbildungen. Bei RX lauten ihre Haupttitel **RX Success — Zeitliche SNR-Evidenz: Target {callsign}** und **RX Success — Zeitliche Evidenz: Target {callsign}**; TX verwendet die entsprechenden **TX Success**-Titel, und beide ergänzen den aktiven Bereich. Die obere Abbildung behält bei RX **Abweichung des erfolgreichen RX-SNR im Zeitverlauf/nach UTC-Stunde** und bei TX **Abweichung des erfolgreichen TX-SNR im Zeitverlauf/nach UTC-Stunde** bei. Jede beitragende TX-Station bei RX beziehungsweise RX-Station bei TX wird auf ihren eigenen Laufmedian zentriert; die gestrichelte `0-dB`-Linie kennzeichnet daher das typische erfolgreiche Niveau dieser Station, während die Medianlinie die Stationen beziehungsweise in der gefalteten Ansicht Stationen und Tage zusammenfasst. Jede Station mit mindestens drei erfolgreichen Target-SNR-Beobachtungen verwendet den Median ihres erfolgreichen SNR über das vollständige gewählte UTC-Fenster als Basislinie. Positive Werte bedeuten, dass erfolgreiche Signale auf ihren jeweiligen Funkwegen stärker als üblich waren; negative Werte bedeuten schwächere Signale. Stationen unterhalb dieser SNR-Schwelle bleiben in der unteren 2×2-Abbildung vollständig enthalten. Über diese untere Abbildung hinweg lauten die gemeinsamen Spaltenüberschriften **Evidenz im Zeitverlauf ({time_bin}-Bins)** und **Evidenz nach UTC-Stunde (1-h-Bins)**. Die gefaltete Stationszeile ergänzt **Durchschnittliche Stationspräsenzen pro berücksichtigtem UTC-Tag**, die gefaltete Gelegenheitszeile **Durchschnittliche bestätigte Gelegenheiten pro berücksichtigtem UTC-Tag**. Die stationsgleichgewichtete obere Zeile behält in beiden Spalten den kurzen y-Achsentitel **TX-Stationen** beziehungsweise **RX-Stationen**. In jedem chronologischen Bin gibt jede beitragende qualifizierende Station eine Gesamtstimme ab, die nach ihrem eigenen Outcome-Verhältnis im Bin aufgeteilt wird: bei RX zwischen `Vom Target gehört` und `Nur von anderen gehört`, bei TX zwischen `Target gehört` und `Nur andere Signale gehört`; die Linie an der rechten Achse zeigt die stationsgleichgewichtete Success Rate. In jedem gefalteten UTC-Stunden-Bin ist die gesamte Höhe des Stationsbalkens die durchschnittliche Zahl verschiedener Stations-Datum-Stunden-Präsenzen über die berücksichtigten Tage, deren Stunde das gewählte Analysefenster überlappt. Eine Station kann an jedem berücksichtigten Datum einmal zählen; eine berücksichtigte Datum-Stunde ohne Evidenz bleibt mit null Stützung im Nenner. Die grün-grauen Komponenten sind eine nach der Rate aufgeteilte Stützungsdarstellung: Sie teilen diese durchschnittliche Stützung mit der unveränderten gefalteten stationsgleichgewichteten Success Rate auf, für die zunächst die Outcomes jeder Station in dieser UTC-Stunde über die berücksichtigten Tage gepoolt werden und danach jede unterschiedliche Station eine gleich große Ratenstimme erhält. Die Komponenten reproduzieren daher die Ratenlinie, sind aber weder Durchschnitte aufgeteilter Stations-Datum-Stunden-Stimmen noch direkte Anzahlen erfolgreicher oder ausschließlicher Gegen-Stationen pro Tag. Die untere Zeile behält in beiden Spalten den kurzen y-Achsentitel **Gelegenheiten**. Sie zählt jede bestätigte Gelegenheit chronologisch einmal und mittelt die Outcome-Anzahlen in der gefalteten Ansicht über dieselben berücksichtigten Tage; die Linie an der rechten Achse zeigt die unveränderte Success Rate auf Beobachtungsebene. Eine gemeinsame Legende unter dem Titel der unteren Abbildung kennzeichnet das grüne Erfolgs-Outcome, das graue Gegen-Outcome und die Success-Rate-Linie. Alle vier rechten Achsen verwenden eine gemeinsame, bei null beginnende Success-Rate-Skala mit gerundeter Reserve; die linken Stützungsachsen skalieren unabhängig. Chronologische Panels erhalten die tatsächliche Laufreihenfolge mit der gewählten Aggregation von `1 h`, `2 h`, `3 h`, `6 h`, `12 h` oder `24 h`. Chronologische `1 h`-Balken, deren Bins an UTC-Stundengrenzen verankert sind, sind in ihren Einheiten direkt mit den gefalteten Durchschnitten vergleichbar; breitere chronologische Bins sind anhand ihrer Höhe nicht direkt vergleichbar. UTC-Stunden-Panels verwenden feste einstündige Bins. Die einzelne Annotation `N UTC-Tage zusammengeführt` nennt die globale Zahl berücksichtigter Tage, während jede Stunde Tage ausschließt, deren Slot außerhalb des gewählten Fensters liegt. Eine berücksichtigte Datum-Stunde innerhalb des Fensters bleibt ohne Evidenz mit null im Nenner. Ein erster oder letzter UTC-Stunden-Slot, der das Fenster nur teilweise überlappt, zählt dennoch als ein vollständiger berücksichtigter Slot statt expositionsanteilig gewichtet zu werden; dadurch kann ein Randstundenmittel niedriger ausfallen. Die Faltung erfordert mindestens zwei UTC-Tage; andernfalls wird die chronologische Spalte erweitert und die lokalisierte Meldung zur nicht verfügbaren Ansicht gezeigt. Leere oder schwach belegte Raten-Bins sind fehlende oder dünne Evidenz und keine Misserfolge.

Lies Peer-Reichweite, die exakten Raten des Segment-Inspektors, beide grün-grauen Zeitstapel und ihre Success-Rate-Linien an der rechten Achse, erfolgreiches SNR und die stützenden Stations- und Opportunity-Anzahlen gemeinsam. Übereinstimmung über gut gestützte Entfernungs-Bins, Stationen, Gelegenheiten und wiederkehrende Zeit-Bins ist stärkere beschreibende Evidenz als eine gepoolte Anzahl oder eine einzelne zeitliche Spitze. Eine Zensierung des erfolgreichen SNR bleibt möglich, weil für verpasste Signale kein Target-SNR vorliegt. Diese Ansichten belegen dennoch weder Unabhängigkeit noch Kalibrierung, Ausbreitungsart oder physische Ursache.
...
`Station Insights` listet die Identitäten `callsign + locator` auf, die zum ausgewählten Segment beitragen. Bei RX folgen auf `TX-Station`, `Locator`, `km` und `Azimut` die Spalten `Vom Target gehört`, `Nur von anderen gehört`, `Bestätigte Gelegenheiten`, `Success Rate (%)` und `Median des erfolgreichen Target-SNR (dB @ 30 dBm)`. Bei TX verwendet die Tabelle `RX-Station`, `Target gehört` und `Nur andere Signale gehört`; die übrigen Spalten bleiben gleich. Lies jede Rate zusammen mit beiden Outcome-Anzahlen und der Tiefe bestätigter Gelegenheiten. Mit `Stationen „Nur von anderen gehört“ anzeigen` bei RX beziehungsweise `Stationen „Nur andere Signale gehört“ anzeigen` bei TX lassen sich qualifizierende Stationen mit ausschließlicher Gegen-Evidenz wieder einblenden.

Wähle genau eine Stationszeile aus, um die `Evidenz der ausgewählten Station` zu öffnen. Die Auswahl einer anderen Zeile ersetzt die bisherige Station, und das Aufheben der Auswahl blendet den Abschnitt aus; Compare erlaubt weiterhin eine oder mehrere ausgewählte Stationen. Eine kompakte Kontextzeile nennt den ausgewählten Funkweg aus `Rufzeichen + Locator` mit seiner Geometrie für den vollständigen Lauf, der Tiefe bestätigter Gelegenheiten, Success Rate und dem Median des erfolgreichen Target-SNR. Auf das etablierte unabhängige Bedienelement für die Zeit-Bin-Breite folgen zwei vollbreite Abbildungen, die das zeitliche System des Segment Inspectors wiederverwenden. **SNR-Evidenz der ausgewählten Station** zeigt das tatsächliche normierte erfolgreiche Target-SNR des Funkwegs: Die chronologische Dichte verwendet jede beibehaltene erfolgreiche Beobachtung und ihren Bin-Median, während das gefaltete UTC-Stundenpanel zunächst einen Median je berücksichtigter Datum-Stunde bildet und diese Mediane danach über die Tage zusammenfasst. **Zeitliche Evidenz der ausgewählten Station** trennt Stationspräsenz von Evidenztiefe und verwendet dieselben richtungsspezifischen Outcomes und Success-Rate-Linien wie die Segmentansicht. Gefaltete UTC-Stundenpanels erfordern mindestens zwei berücksichtigte UTC-Tage; andernfalls werden die chronologischen Panels erweitert und der Hinweis zur nicht verfügbaren Ansicht erscheint. Auswahländerungen verwenden beibehaltene Evidenz und starten keine Provider-Abfrage erneut.
...
* RX Success zeigt `Vom Target gehört`, `Nur von anderen gehört` und `Vom Target gehört, aber nicht unabhängig bestätigt`.
* TX Success zeigt `Target gehört`, `Nur andere Signale gehört` und `Target gehört, RX-Aktivität nicht unabhängig bestätigt`.
...
* bei Success die Anzahl qualifizierender Stationen und bestätigter Opportunities beziehungsweise bei Compare die Anzahl von Joint-Stationen und Joint-Spots/-Paaren;
...
> Für dieses Target, Band, UTC-Zeitfenster und die ausgewählte Peer-Population beschreibt die angezeigte Success Rate den Anteil der unabhängig bestätigten Opportunities, in denen auch das Target qualifizierende Evidenz lieferte. Qualifizierende Stationen, bestätigte Opportunities, geografischer Bereich und Zeitansichten beschreiben Breite, Tiefe und Wiederkehr der stützenden Evidenz.
...
Da WSPRadar noch vor dem Produktivbetrieb steht, bleibt der Konfigurationsvertrag bei Schemaversion 1. Die Referenzstation verlangt `Referenz-Rufzeichen` plus ihr unabhängiges vierstelliges `Referenz-Grid-4`. RX und simultanes TX Hardware A/B verlangen das abweichende `Referenz-Rufzeichen`, leiten ihr gemeinsames Grid-4 jedoch aus dem Target-QTH ab und speichern deshalb kein redundantes `reference_qth`; TX Hardware A/B verlangt außerdem die Felder der gewählten Methode. Jede Compare-Konfiguration speichert sowohl den Zweck der Korrektur als auch ihren Zahlenwert: `no_offset` und `establish_offset` verlangen `0,0 dB`, während `established_offset` eine dokumentierte Korrektur mit Vorzeichen speichert und ausdrücklich auch eine ermittelte Korrektur von `0,0 dB` enthalten darf. Success-Konfigurationen lassen beide Felder weg. Frühere, nicht veröffentlichte v1-Prototypen werden nicht migriert; eine zutreffende Compare-Datei ohne ausdrücklichen Korrekturzweck wird abgelehnt, statt diesen aus einem mehrdeutigen Zahlenwert null zu erraten. Speichere sie mit den aktuellen Bedienelementen neu oder erstelle sie neu.
...
| **Referenzseitige SNR-Korrektur (dB)** | `0,0 dB`; Wertebereich `-99,9` bis `+99,9 dB` | Wird zum SNR der Referenzseite addiert, bevor Delta SNR berechnet wird. Geführt zeigt den Zahlenwert für `Ermittelte Korrektur verwenden`; Klassisch zeigt ihn für jedes Compare-Design. Eine von null verschiedene Änderung in Klassisch wählt `established_offset`, während `0,0 dB` den aktuellen ausdrücklichen Modus beibehält. Läufe ohne Offset und Offset-Ermittlungsläufe halten den Wert bei `0,0 dB`; eine ermittelte Korrektur darf ein Vorzeichen haben oder ausdrücklich als `0,0 dB` ermittelt worden sein. |
...
- `Stationen „Nur von anderen gehört“ anzeigen` bei RX beziehungsweise `Stationen „Nur andere Signale gehört“ anzeigen` bei TX stellt qualifizierende Success-Identitäten mit ausschließlicher Gegen-Evidenz wieder dar. Die Einstellung wird für Success gespeichert.
...
- Die Stationsauswahl verändert die Abbildungen der ausgewählten Station und den gewählten Drill-Down. Success speichert keine oder genau eine exakte Identität aus `Rufzeichen + Locator`; die Auswahl einer anderen Zeile ersetzt die aktuelle Identität. Enthält ein historischer Success-Zustand mehrere Identitäten, behält die Wiederherstellung die erste gültige Identität in gespeicherter Reihenfolge bei, verwirft die zusätzlichen Identitäten, ersetzt sie nie durch eine andere Station und stellt keine Auswahl wieder her, wenn keine Identität gültig bleibt. Die Compare-Auswahl bleibt unabhängig und kann eine oder mehrere exakte Identitäten enthalten. Werden alle Compare-Stationen ausgewählt, speichert die Konfiguration diese Absicht, statt jede aktuelle Tabellenzeile einzeln aufzuführen; bei einem gleitenden Fenster `Letzte X Stunden` kann sich die rekonstruierte Stationsmenge daher mit der Evidenz ändern. Fehlt eine explizit gespeicherte Compare-Identität im aktuellen Segmentbereich, bleibt sie mit einem Hinweis unausgewählt und wird nicht ersetzt; ihre gespeicherte Identität bleibt bis zu einer neuen Tabellenauswahl erhalten, sodass sie nach Wahl des passenden Segmentbereichs wieder verfügbar werden kann.
- Bei jeder ausgewählten Success-Station verändert die Aufforderung `↓ Zeitliche Aggregationsbreite wählen:` die chronologischen Panels in **SNR-Evidenz der ausgewählten Station** und **Zeitliche Evidenz der ausgewählten Station**. Ihre gefalteten UTC-Stundenpanels bleiben auf feste einstündige Bins eingestellt. Ein weiterhin unterstütztes gespeichertes Success-Bin bleibt bei einem Wechsel der Stationsidentität erhalten, bleibt vom Zeit-Bin des Segment Inspectors unabhängig und startet die abgeschlossene Provider-Analyse nie erneut. Compare behält sein unabhängiges Bin für ausgewählte Stationen.
...
- Bei Success steht die Aufforderung `↓ Zeitliche Aggregationsbreite auswählen` unmittelbar über den Optionen `1 h`, `2 h`, `3 h`, `6 h`, `12 h` und `24 h` unter `Zeitliche Evidenz`. Dieses Bedienelement auf Segmentebene ändert nur das chronologische Segmentpanel. Das nach UTC-Stunden gefaltete Panel bleibt auf feste einstündige Klassen eingestellt; keine der beiden Ansichten verändert die abgeschlossene Analyse oder die unabhängige spätere Zeitachse ausgewählter Stationen.
- Leere Success-Zeit- oder Entfernungs-Bins bleiben leer; sie werden nicht zu Evidenz mit einer Rate von null umgedeutet.
...
| Symptom | Nächste Prüfungen |
|---|---|
| **Viele Success-Zeilen ohne unabhängige Bestätigung** | RX bezeichnet diese Zeilen als `Vom Target gehört, aber nicht unabhängig bestätigt`; TX als `Target gehört, RX-Aktivität nicht unabhängig bestätigt`. Sie bleiben prüfbar, gehen jedoch nicht in die Success Rate ein. |
...
Jede Referenzstation wird anhand des exakten Rufzeichens plus ihres unabhängig konfigurierten, exakt vierstelligen Referenz-Grid-4 zugeordnet. RX und simultanes TX Hardware A/B leiten dagegen beide deaktivierten Grid-4-Anzeigen aus den ersten vier Zeichen des Target-QTHs ab und führen keine unabhängige Referenz-QTH-Einstellung; sequenzielles TX Hardware A/B verwendet auf beiden Zeitplanseiten das gemeinsame exakte Target-Rufzeichen und Target-Grid-4. Lokale Kandidaten werden weiterhin geografisch ausgewählt.
...
| Buddy-Compare | exaktes Target-Rufzeichen plus Grid-4 des Target-QTH | exaktes Referenzrufzeichen plus unabhängiges Referenz-Grid-4; entferntes Rufzeichen + gemeldeter Locator | ein konsolidierter Peer-Zyklus |
...
Success und alle Compare-Modi verwenden das exakte Target-Rufzeichen plus die ersten vier Zeichen des konfigurierten Target-QTHs. Ein sechsstelliges Target-QTH bleibt außerhalb der Archivauswahl bedeutsam, weil sein vollständiger Wert Karten, lokale Radiusgeometrie, Azimut/Entfernung und Sonnenstandsberechnungen verankert. Die Referenzstation verwendet ein unabhängiges exaktes Referenzrufzeichen plus ein exakt vierstelliges Referenz-Grid-4. Hardware A/B leitet das gemeinsame Grid-4 aus dem Target-QTH ab und speichert kein separates Referenz-QTH. Grid-6 ist somit kein Abfrageselektor: `JN37AA` und `JN37XX` wählen beide `JN37`, `JN38` dagegen nicht. Die gemeinsame Hardware-A/B-Zuordnung zum Grid-4 kann den tatsächlichen Betrieb am selben physischen QTH nicht nachweisen.
...
Die Klassifikation der Success Rate selbst wird nicht leistungsnormiert. Das daneben angezeigte SNR erfolgreicher Target-Evidenz wird auf die gemeldete Leistung von 30 dBm normiert.
...
Auf Compare-Karten ordnen die Kategorien unter `STATIONS` Identitäten zu; die Kategorien unter `SPOTS` zählen das Evidenzvolumen. Success-Karten zeigen `GELEGENHEITEN` oberhalb von `STATIONEN` und verwenden in beiden Zeilen dieselben richtungsspezifischen Anzeigebezeichnungen. Die obere Zeile zählt kanonische Target- und Gegen-Evidenz-Outcomes im Success-Nenner; die untere ordnet qualifizierende RX-Identitäten `Vom Target gehört` oder `Nur von anderen gehört` und qualifizierende TX-Identitäten `Target gehört` oder `Nur andere Signale gehört` zu. Ein `Joint Spot` ist eine konsolidierte Vergleichseinheit desselben Zyklus und nicht zwingend eine einzelne unveränderte Datenbankzeile.
...
* Bewahre das Analyse-Exportpaket auf und dokumentiere die Evidenz, auf der die Schlussfolgerung tatsächlich beruht:
    ...
    * bei Success die qualifizierenden Stationen und bestätigten Opportunities hinter der angezeigten Gewichtung und dem angezeigten Umfang;
...
Bei Success enthält `figure_segment_insight.png` das richtungsspezifische Panel zur Mindestens-einmal-Stationsreichweite, **RX/TX Success Rate nach Entfernung der TX-/RX-Station** und **Erfolgreiches Target-SNR nach Entfernung der TX-/RX-Station** für den aktiven Inspector-Bereich. `figure_segment_temporal_snr_deviation.png` enthält die chronologische und nach UTC-Stunde gefaltete Abbildung **RX/TX Success — Zeitliche SNR-Evidenz**. `figure_segment_temporal_evidence.png` enthält die daran ausgerichtete untere Abbildung **RX/TX Success — Zeitliche Evidenz** mit den gemeinsamen Spaltenüberschriften **Evidenz im Zeitverlauf ({time_bin}-Bins)** und **Evidenz nach UTC-Stunde (1-h-Bins)**, kurzen y-Achsentiteln für Stationen und Gelegenheiten, gefalteten Untertiteln je berücksichtigtem UTC-Tag, richtungsspezifischen grün-grauen Outcome-Stapeln, vier gemeinsamen skalierten Success-Rate-Achsen und einer gemeinsamen Legende. Ihre gefalteten Untertitel kennzeichnen durchschnittliche Stations-Datum-Stunden-Stützung und durchschnittliche Gelegenheiten pro UTC-Tag; ihre chronologischen Stapel bleiben Anzahlen je gewähltem Zeit-Bin. Tragen weniger als zwei UTC-Tage bei, werden die chronologischen Panels erweitert und der Hinweis zur nicht verfügbaren gefalteten Ansicht bleibt erhalten.

Die zwei stabilen Dateinamen für die ausgewählte Success-Station bewahren die gemeinsam genutzten Zeitansichten getrennt. `figure_selected_station_snr_evidence.png` enthält die vollbreite **SNR-Evidenz der ausgewählten Station** mit tatsächlichem normiertem erfolgreichem Target-SNR, jeder erfolgreichen Beobachtung in der chronologischen Dichte und einem Median je berücksichtigter Datum-Stunde in der gefalteten Dichte. `figure_selected_station_temporal_evidence.png` enthält die vollbreite **Zeitliche Evidenz der ausgewählten Station** mit Stationspräsenz, Evidenztiefe der Gelegenheiten und den identischen Success-Rate-Reihen für eine Station. Beide verwenden dieselbe ausgewählte Identität sowie dieselben Zeitrezepte und Renderer wie ihre Browser-Vorschau; beide lassen die gefalteten UTC-Stundenpanels aus, wenn weniger als zwei berücksichtigte Tage beitragen. Bei Compare reproduziert der etablierte Dateiname `figure_selected_station_evidence.png` weiterhin die für die ausgewählten Stationen aktive Zeitansicht zum Zeitpunkt der Exportvorbereitung. Der Modus `Chronologisch` verwendet die gewählte Compare-Zeitklasse; `UTC-Stunde` verwendet feste einstündige Zeitfenster und dieselben ausgewählten Evidenzzeilen. Der Compare-Modus wird in der gespeicherten `.config` und in `run_metadata.json` festgehalten.

Die gespeicherte Konfiguration enthält die zutreffenden ausführbaren Einstellungen. `run_metadata.json` erfasst automatisch Anwendungsname und -version, Exportzeit, Sprache, Richtung, Band, Benchmark-Auswahl, konfigurierte Zeitauswahl, Modus und Zahlenwert der referenzseitigen Korrektur als `benchmark_snr_correction_mode` und `benchmark_snr_correction_db`, Filter, Schwellen, Ergebnisblöcke und Auswahlen in der Inspektionsansicht. Für ausgewählte Success-Evidenz befüllt die Metadatei außerdem `selected_station_label`, `selected_station_context`, `selected_station_count`, `selected_station_role`, `selected_evidence_weighting` und die stabile Zuordnung `selected_evidence_figures` von Dateinamen zu Beschreibungen; sie erfasst die eine exakte ausgewählte Identität und die Anzahl eins. Compare erfasst seine exakten Identitäten in `selected_stations`, die Auswahlanzahl sowie die Felder des aktiven Evidenzrezepts und der Zeitansicht; die optionalen beschreibenden Success-Felder bleiben unbelegt. Die Browser-Darstellung behält die etablierte Regel bei: eine genaue Identität, die namentlich genannten Identitäten bei zwei bis fünf Auswahlen oder oberhalb von fünf nur die richtungsspezifische Anzahl ausgewählter Stationen.
...
1. Weise einer Instanz das exakte Target-Rufzeichen und Target-QTH und der anderen das davon verschiedene exakte Referenz-Rufzeichen zu. Konfiguriere die Referenzinstanz für Meldungen vom selben Test-QTH; WSPRadar zeigt deaktivierte Target- und Referenz-Grid-4-Felder an, die beide aus den ersten vier Zeichen des Target-QTHs abgeleitet werden, und ordnet beide hochgeladenen Identitäten diesem gemeinsamen Grid-4 zu.
