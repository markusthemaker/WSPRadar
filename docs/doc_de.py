...
Die Evidenz der ausgewählten Station in Compare filtert die beibehaltenen Zeilen des aktiven Bereichs auf genau eine ausgewählte Identität aus `callsign + locator` und bereitet aus derselben Zeit-Bin-Steuerung zwei vollbreite Abbildungen auf. Die erste Abbildung behält die etablierten Ansichten des absoluten Delta SNR. **Δ SNR im Zeitverlauf** ordnet jeden beibehaltenen Joint Spot oder jedes Scheduled Pair dieses Funkwegs mit der gewählten Aggregationsbreite in seiner tatsächlichen UTC-Folge an. **Δ SNR nach UTC-Stunde** faltet dieselben Evidenzzeilen auf Beobachtungsebene aus allen berücksichtigten Tagen in feste einstündige UTC-Slots. Jedes Panel normiert seine Dichte unabhängig auf die am stärksten belegte Zelle; beide verwenden jedoch dieselbe medianzentrierte, nichtlineare Delta-SNR-Achse des ausgewählten Funkwegs mit absoluten dB-Beschriftungen.
