...
* Both temporal panels use the map's fixed nonlinear Success Rate scale and identical distance/time axes. Distance is labeled at the `2500`, `5000`, `10000`, `15000` and `20000 km` ring transitions rather than with long range-bin strings. Time bins are contiguous, anchored at the exact analysis start and extend to the analysis end; empty bins remain blank rather than disappearing, and at least six time labels are shown when enough bins exist.
