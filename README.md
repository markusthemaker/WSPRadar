...
| **Start Date (UTC)**, **End Date (UTC)**, **Start Time (UTC)**, **End Time (UTC)** | exact default boundaries above | Dates start at 2008 and one window is limited to 31 elapsed days. Editing Start Date suggests an End Date seven days later, capped at the current UTC date. The End Date picker is bounded by the selected Start Date, the 31-day limit and today; exact date-and-time violations are shown immediately and keep `Run` unavailable. Edited values are quantized down to 15-minute boundaries and the effective values are written back to the controls. |
...
The two station-population exclusions use result-specific interactive defaults: an untouched Performance setup starts with both exclusions on, while an untouched Compare setup starts with both off. Editing either toggle makes that field an explicit session choice that is retained across result-type changes. Loaded configurations, demos and analysis URLs always retain their explicit saved values instead of receiving interactive defaults.

**`Exclude Special Callsigns Q, 0, 1`**

- **Default:** on for Performance; off for Compare, unless explicitly set by the operator or a loaded configuration
...
The white connected markers remain a separate statistic: the median within each populated time bin. A subtle IQR band bounded by fine Q1 and Q3 lines uses the same raw Joint-Spot or complete-Scheduled-Pair values before integer heatmap binning or the nonlinear display transform. It appears only from five contributed values onward, breaks at unsupported bins and leaves sparse medians visible. The band describes the middle 50% of a bin, not uncertainty or a confidence interval, and does not alter the full finite evidence envelope used by the axis.

Let `M` denote that scope's exact evidence median. For a broad range, equal visual steps are anchored at `M`, `M +/- 3`, `M +/- 6`, `M +/- 10`, `M +/- 20` and `M +/- 30 dB`; an unlabelled tail anchor continues at `M +/- 60 dB` and extrapolates farther when necessary. If every required deviation from `M` is at most `10 dB`, the tighter visible anchors are `M`, `M +/- 1`, `M +/- 3`, `M +/- 6` and `M +/- 10 dB`; unlabelled `M +/- 20` and `M +/- 40 dB` anchors define the compressed continuation outside that visible range. The required deviation includes the applicable raw histogram or rounded heatmap-bin edges, a minimum 3 dB half-span and absolute `0 dB`, so tails and the Target-equals-Reference equality value remain within the scale even though no dedicated zero reference line is drawn.

Tick labels show the resulting **absolute Delta SNR**, not distance from `M`. For example, `M = +6 dB` produces the broad labels `-24, -14, -4, 0, +3, +6, +9, +12, +16, +26, +36 dB`. The red median line and legend identify which scale position is `M`; the tick label itself does not append `M`.
...
Each provider query accepts at most 1,000,000 complete result rows. WSPRadar requests one additional sentinel row only to detect a larger result; if it appears, the run stops before processing and no truncated partial result is analyzed. Shorten the date or UTC-time window to reduce the query result. Enabling `Exclude Special Callsigns Q, 0, 1` can also reduce rows because that filter is part of the provider SQL. `Maximum peer distance from Target (km)`, `Exclude Moving Stations`, solar-state and evidence filters operate after retrieval and therefore cannot prevent this safety stop. For a Local Neighborhood comparison, reducing `Neighborhood Radius (km)` can reduce rows because that radius is applied in provider SQL.

`Solar state at Target QTH` uses solar elevation at Target QTH. Normal same-cycle evidence uses its cycle timestamp. Automatic scheduled TX A/B uses the midpoint between the two planned starts, so Target and Reference in one pair cannot be split into different solar classes.
...
For Performance, `figure_segment_insight.png` contains the direction-specific at-least-once station-reach panel, **RX/TX Decode Rate by TX-/RX-Station Distance**, and **Successful Target SNR by TX-/RX-Station Distance** for the active Inspector scope. `figure_segment_temporal_snr_deviation.png` contains the chronological/UTC-hour **RX/TX Performance Temporal SNR Evidence** figure, including its supported IQR bands bounded by fine Q1/Q3 lines. `figure_segment_temporal_evidence.png` contains the aligned lower **RX/TX Performance Temporal Evidence** figure with shared **Evidence over Time ({time_bin} bins)** and **Evidence by UTC Hour (1 h bins)** column headers, short chronological and folded station/opportunity y-axis labels, direction-specific green/grey outcome stacks, four common-scale Decode Rate axes and one shared legend. Although its short folded y-axis labels omit the denominator, the values remain average station-date-hour support and average opportunities per UTC date; its chronological stacks remain counts per selected time bin. When fewer than two UTC dates contribute, exported folded panels retain their normal geometry without folded data and show the localized boxed unavailable notice, while the chronological panels retain their normal width.

The two stable filenames for a station selected in Performance preserve the shared temporal views independently. `figure_selected_station_snr_evidence.png` contains the full-width **Selected Station SNR Evidence** figure with actual normalized successful Target SNR, every successful observation in the chronological density, one median per represented date-hour in the folded density and supported IQR bands bounded by fine Q1/Q3 lines in both panels. `figure_selected_station_temporal_evidence.png` contains the full-width **Selected Station Temporal Evidence** figure with station presence, opportunity depth and their equal one-station Decode Rate series. Both use the same selected identity, temporal recipes and renderers as their browser previews. For Compare, `figure_segment_temporal_evidence.png` remains the absolute paired Delta SNR timeline and `figure_segment_temporal_coverage.png` contains the station/outcome coverage view. `figure_selected_station_evidence.png` contains the selected path's absolute Delta SNR panels; both absolute Compare PNGs include supported IQR bands bounded by fine Q1/Q3 lines. `figure_selected_station_coverage.png` contains its Only Target, Joint and Only Reference coverage plus Joint Evidence Share. The applicable figures use the same browser recipes. All applicable Performance and Compare exports retain the normal folded-panel geometry when fewer than two represented dates contribute, omit only the folded data, and show the same localized boxed unavailable notice as their browser previews; chronological panels do not expand.
