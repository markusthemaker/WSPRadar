...
T = {
    "en": {
        ...
        'fig_compare_coverage_title_rx': "RX Compare Temporal Evidence Coverage: Target {callsign}",
        'fig_compare_coverage_title_tx': "TX Compare Temporal Evidence Coverage: Target {callsign}",
        ...
        'fig_compare_coverage_unit_folded_y_rx': "Avg. Transmitter-\nCycles / Represented\nUTC Date",
        'fig_compare_coverage_unit_folded_y_tx': "Avg. Receiver-Cycles\n/ Represented\nUTC Date",
        'fig_compare_coverage_unit_folded_y_scheduled': "Avg. Scheduled A/B\nPairs / Represented\nUTC Date",
        ...
        'fig_selected_compare_coverage_title_rx': "RX Compare Selected Path Evidence Coverage: {station} ({locator})",
        'fig_selected_compare_coverage_title_tx': "TX Compare Selected Path Evidence Coverage: {station} ({locator})",
        ...
        'fig_selected_compare_coverage_unavailable': "No retained comparison outcomes are available for this selected path.",
        ...
        'fig_success_iqr': "IQR",
        'fig_success_two_station_range': "Range (2 stations)",
        ...
        'fig_temporal_bin_iqr': "Bin Q1–Q3 (middle 50%)",
        ...
        "hlp_benchmark_offset_db": "Added to the reference-side SNR before \u0394 SNR is calculated. Applies to compare maps only. Buddy/local benchmarks use the reference side; Hardware A/B uses the Reference identity or Reference schedule. Enter decimal values with a point. If a calibration run shows target-reference = +1.6 dB, enter +1.6 dB.",
    ...
    "de": {
        ...
        'fig_rx_comp_temporal_prefix': "RX Compare – Zeitverlauf",
        'fig_tx_comp_temporal_prefix': "TX Compare – Zeitverlauf",
        ...
        'fig_compare_coverage_title_rx': "RX Compare — Zeitliche Evidenzabdeckung: Target {callsign}",
        'fig_compare_coverage_title_tx': "TX Compare — Zeitliche Evidenzabdeckung: Target {callsign}",
        ...
        'fig_compare_coverage_station_folded_y_rx': "Ø TX-Stationen je\nberücksichtigtem\nUTC-Tag",
        'fig_compare_coverage_station_folded_y_tx': "Ø RX-Stationen je\nberücksichtigtem\nUTC-Tag",
        ...
        'fig_compare_coverage_unit_folded_y_rx': "Ø Senderzyklen je\nberücksichtigtem\nUTC-Tag",
        'fig_compare_coverage_unit_folded_y_tx': "Ø Empfängerzyklen je\nberücksichtigtem\nUTC-Tag",
        'fig_compare_coverage_unit_folded_y_scheduled': "Ø geplante A/B-Paare je\nberücksichtigtem\nUTC-Tag",
        ...
        'fig_selected_compare_coverage_title_rx': "RX Compare — Evidenzabdeckung des ausgewählten Funkwegs: {station} ({locator})",
        'fig_selected_compare_coverage_title_tx': "TX Compare — Evidenzabdeckung des ausgewählten Funkwegs: {station} ({locator})",
        ...
        'fig_selected_compare_coverage_unit_folded_y_simultaneous': "Ø WSPR-Zyklen je\nberücksichtigtem\nUTC-Tag",
        'fig_selected_compare_joint_share': "Joint-Evidenzanteil",
        'fig_selected_compare_coverage_unavailable': "Für diesen ausgewählten Funkweg sind keine beibehaltenen Compare-Outcomes verfügbar.",
        ...
        'fig_success_median': "Median",
        'fig_success_iqr': "IQR",
        'fig_success_two_station_range': "Spanne (2 Stationen)",
        ...
        'fig_success_station_support_folded_y_rx': "Ø TX-Stationen je\nberücksichtigtem\nUTC-Tag",
        'fig_success_station_support_folded_y_tx': "Ø RX-Stationen je\nberücksichtigtem\nUTC-Tag",
        'fig_success_opportunities_y': "Gelegenheiten",
        'fig_success_opportunities_folded_y': "Ø Gelegenheiten je\nberücksichtigtem\nUTC-Tag",
        ...
        'fig_temporal_bin_iqr': "Q1–Q3 je Bin (mittlere 50 %)",
...
RESULT_GUIDANCE = {
    "en": {
        ...
        "sections": {
            ...
            "selected_compare_joint": {
                "read": """Selected Station Evidence isolates one chosen {peer_type} station and one specific radio path. **Δ SNR over Time** uses the selected chronological bin; **Δ SNR by UTC Hour** folds the path into 1-hour slots. Values are signed and not baseline-normalized: positive favors Target, negative favors Reference and 0 dB is equality. Red dashed line: median of all Joint Spots; pale markers: bin medians. Fine pale Q1–Q3 rails show the middle 50% of that same bin population only when at least five Joint Spots contribute. Color shows relative Joint-Spot density per panel, not advantage; height gives labelled dB on the nonlinear axis. Gaps mean no Joint evidence. <strong class="defined-term">Selected Path Evidence Coverage</strong> adds Only Target, Joint and Only Reference. Bars count **Retained WSPR Cycles**; the amber line shows their Joint share. Folded bars average per represented UTC date. A stable sign, dense evidence and high Joint share support a persistent path difference; reversals or long one-sided periods indicate dependence on time or conditions.""",
                "limits": """This is one observation-weighted path, not the station-balanced segment result. Simultaneous Compare retains only cycles with observed Target activity. RX: Target decoded a qualifying signal; TX: Target was decoded somewhere. Reference has no equivalent gate. Target-offline cycles are excluded; missing Reference may become Only Target. Only Joint units supply ΔSNR. One-sided counts favor Target under this gate and show availability, not symmetric wins and losses. Swapping roles can change coverage; paired values reverse sign. The gate cannot alter Joint evidence because every Joint Spot contains both sides. Q1–Q3 describes within-bin spread, not uncertainty or a confidence interval. UTC recurrence does not establish cause.""",
            },
            "selected_compare_scheduled": {
                "read": """Selected Station Evidence isolates one RX station and one scheduled radio path. **Δ SNR over Time** uses the selected chronological bin; **Δ SNR by UTC Hour** folds the path into 1-hour slots. Values are signed and not baseline-normalized: positive favors Target, negative favors Reference and 0 dB is equality. The red dashed line is the median of complete Scheduled Pairs; pale markers are bin medians. Fine pale Q1–Q3 rails show the middle 50% of that same bin population only when at least five complete pairs contribute. Color shows relative Scheduled-Pair density per panel, not advantage; height gives labelled dB on the nonlinear axis. Gaps mean no complete pair was available. <strong class="defined-term">Selected Path Evidence Coverage</strong> adds Only Target, Joint and Only Reference. Bars count **Scheduled A/B Pairs**; the amber line shows the complete-pair share. Folded bars average per represented UTC date. A stable sign, dense evidence and high completed-pair share support a persistent path difference; reversals or long one-sided periods indicate dependence on time or conditions.""",
                "limits": """This is one observation-weighted receiver path, not the station-balanced segment result. Scheduled Compare follows the configured planned Target–Reference pairs. Pair ΔSNR exists only when both sides were decoded. One-sided units show incomplete-pair coverage, contain no missing-side SNR and are not symmetric wins or losses. Time separation allows propagation, QRM, switching and schedule phase to resemble a path difference. Q1–Q3 describes within-bin spread, not uncertainty or a confidence interval. UTC recurrence does not establish cause.""",
            },
            "selected_success_rx": {
                "read": """Selected Station Evidence isolates one remote TX station from the active RX Performance scope. Selecting another Station Insights row replaces the current station. The upper <strong class="defined-term">Selected Station SNR Evidence</strong> figure uses the same chronological and folded UTC-hour layout as Temporal Evidence, but shows this path’s actual normalized successful Target SNR rather than deviation from a station baseline. Chronological color shows where successful SNR observations concentrate in each selected time bin, and the line shows their bin median. The UTC-hour panel first gives every represented date one hourly median, then shows how those values recur across dates. Fine pale Q1–Q3 rails bound the middle 50% of those same populations: raw successful observations chronologically and date-hour medians when folded. They appear only with at least five values. The lower <strong class="defined-term">Selected Station Temporal Evidence</strong> figure separates station presence from opportunity depth. When the selected TX station contributes evidence in a chronological bin, it supplies one split station vote between Heard by Target and Heard by others only. The folded station row shows its average presence per represented UTC date. The opportunity row counts every confirmed opportunity chronologically and shows average counts per represented date after UTC-hour folding. Green shares and Decode Rate lines show the same selected-path rate. Because exactly one station is selected, station-balanced and Opportunity-level Decode Rates are identical; matching lines are expected. Read SNR and Decode Rate together: strong successful decodes with a falling rate can mean weaker signals were no longer decoded.""",
    ...
    "de": {
        ...
        "sections": {
            ...
            "temporal_evidence_joint": {
                "read": """Zeitliche Evidenz verbindet ΔSNR und Evidenzbasis. **Δ SNR im Zeitverlauf** nutzt gewählte Binbreite; **Δ SNR nach UTC-Stunde** faltet Tage in 1-h-Slots. Positiv spricht für Target, negativ für Referenz; 0 dB bedeutet Gleichheit. Rote gestrichelte Linie: Median aller Joint Spots; helle Marker: Bin-Mediane. Feine helle Q1–Q3-Linien zeigen die mittleren 50 % derselben Bin-Population nur, wenn mindestens fünf Joint Spots beitragen. Farbe: relative Joint-Spot-Dichte je Panel, nicht |ΔSNR| oder Vorteil; Höhe: dB-Wert auf nichtlinearer Achse. <strong class="defined-term">Zeitliche Evidenzabdeckung</strong> zeigt Only Target, Joint und Only Reference. Oben liefert jede Station eine aufgeteilte Stimme; die Balkenhöhe zeigt beitragende Stationen, die blaue Linie deren gleichgewichteten Joint-Anteil. Unten: Vergleichseinheiten; gelbe Linie: Joint-Anteil der Einheiten. Linienabstand zeigt Volumengewichtung. Gefaltete Balken mitteln je UTC-Stunde über berücksichtigte Tage. Stabiles Vorzeichen, dichte Evidenz und ähnliche Linien stützen Wiederkehr; Spitzen, Wechsel oder geringer Joint-Anteil mahnen zur Vorsicht.""",
                "limits": """Simultaner Compare nutzt nur Zyklen mit beobachteter Target-Aktivität: RX: Target decodierte ein qualifizierendes Signal; TX: Target wurde irgendwo decodiert. Es gibt kein entsprechendes Referenz-Gate. Target-Offline-Zyklen entfallen; fehlende Referenz-Evidenz kann Only Target werden. Einseitige Abdeckung ist daher Target-begünstigt und zeigt Verfügbarkeit, keine symmetrischen Siege oder Niederlagen. Joint-ΔSNR bleibt unverändert, da Joint Spots beide Seiten enthalten. Q1–Q3 beschreibt die Streuung innerhalb eines Bins, nicht die Unsicherheit oder ein Konfidenzintervall. UTC-Wiederkehr belegt keine Ursache.""",
            },
            "temporal_evidence_scheduled": {
                "read": """Zeitliche Evidenz verbindet Pair-ΔSNR und Evidenzbasis. **Δ SNR im Zeitverlauf** nutzt gewählte Binbreite; **Δ SNR nach UTC-Stunde** faltet Tage in 1-h-Slots. Positive Werte sprechen für Target, negative für Referenz; 0 dB bedeutet Gleichheit. Rote gestrichelte Linie: Median vollständiger geplanter Paare; helle Marker: Bin-Mediane. Feine helle Q1–Q3-Linien zeigen die mittleren 50 % derselben Bin-Population nur, wenn mindestens fünf vollständige Paare beitragen. Farbe: relative Dichte geplanter Paare je Panel, nicht Vorteil; Höhe: dB-Wert auf nichtlinearer Achse. <strong class="defined-term">Zeitliche Evidenzabdeckung</strong> zeigt Only Target, Joint und Only Reference. Oben liefert jede RX-Station eine aufgeteilte Stimme; die Balkenhöhe zeigt beitragende Empfänger, die blaue Linie deren gleichgewichteten Anteil vollständiger Paare. Unten: geplante A/B-Einheiten; gelbe Linie: Anteil vollständiger Paare. Linienabstand zeigt Volumengewichtung. Gefaltete Balken mitteln je UTC-Stunde über vertretene Tage. Stabiles Vorzeichen, dichte Evidenz und ähnliche Linien stützen Wiederkehr; Spitzen, Wechsel oder geringer Paaranteil mahnen zur Vorsicht.""",
                "limits": """Scheduled Compare folgt den konfigurierten geplanten Target–Referenz-Paaren und nicht dem Target-Active Gate des simultanen Modus. Pair-ΔSNR entsteht nur, wenn beide geplanten Seiten decodiert wurden. Only Target und Only Reference zeigen unvollständige Paare, ohne SNR der anderen Seite; sie sind keine symmetrischen Siege oder Niederlagen. Der Zeitabstand lässt Ausbreitung, QRM, Umschaltung und Zeitplanphase in Pair-ΔSNR einfließen. Q1–Q3 beschreibt die Streuung innerhalb eines Bins, nicht die Unsicherheit oder ein Konfidenzintervall. UTC-Wiederkehr belegt keine Ursache.""",
            },
            "success_evidence_rx": {
                "read": """Performance-Evidenz betrachtet die qualifizierende TX-Stationspopulation auf drei ergänzende Arten nach der berechneten Entfernung vom Target-QTH. <strong class="defined-term">Vom Target mindestens einmal gehörte TX-Stationen nach Entfernung</strong> zeigt, ob ein qualifizierender Funkweg im gewählten UTC-Zeitraum überhaupt einmal zustande kam. <strong class="defined-term">RX Dekodierrate nach Entfernung der TX-Station</strong> zeigt, wie zuverlässig bestätigte Gelegenheiten vom Target gehört wurden, und vergleicht gleiche Stationsgewichtung mit gepoolter Gelegenheitsgewichtung. <strong class="defined-term">Erfolgreiches Target-SNR nach Entfernung der TX-Station</strong> gibt zunächst jeder TX-Station einen Median ihres erfolgreichen SNR und fasst danach diese Stationsmediane in jedem exakten Entfernungs-Bin zusammen. Lies die Panels von links nach rechts: Pfadverfügbarkeit, wiederholte Zuverlässigkeit und Signalstärke bei erfolgreichem Decode. Stations- und Gelegenheitszahlen zeigen die Evidenz hinter jedem Entfernungs-Bin.""",
            ...
            "success_evidence_tx": {
                "read": """Performance-Evidenz betrachtet die qualifizierende RX-Stationspopulation auf drei ergänzende Arten nach der berechneten Entfernung vom Target-QTH. <strong class="defined-term">RX-Stationen, die das Target mindestens einmal hörten, nach Entfernung</strong> zeigt, ob das Target den jeweiligen Teil der aktiven Empfängerpopulation im gewählten UTC-Zeitraum überhaupt erreichte. <strong class="defined-term">TX Dekodierrate nach Entfernung der RX-Station</strong> zeigt, wie zuverlässig das Target gehört wurde, und vergleicht gleiche Stationsgewichtung mit gepoolter Gelegenheitsgewichtung. <strong class="defined-term">Erfolgreiches Target-SNR nach Entfernung der RX-Station</strong> gibt zunächst jeder RX-Station einen Median erfolgreicher Target-Reports und fasst danach diese Stationsmediane in jedem exakten Entfernungs-Bin zusammen. Lies die Panels von links nach rechts: Reichweite, wiederholte Zuverlässigkeit und Signalstärke bei erfolgreichem Decode. Stations- und Gelegenheitszahlen zeigen die Evidenz hinter jedem Bin.""",
...
- **Joint evidence** means that comparable evidence is available for both sides: the same transmission was decoded by both RX sides, corresponding Target and Reference reports were made at the same receiving station, or a scheduled Target–Reference pair is available for sequential TX.""",
        ...
        "messages": {
            ...
            "result_success": "Performance only — stand-alone Target values",
...
- **Joint-Evidenz** bedeutet, dass für beide Seiten vergleichbare Daten vorliegen: dieselbe Aussendung wurde von beiden RX-Seiten dekodiert, an derselben Empfangsstation liegen passende Reports für Target und Referenz vor, oder bei sequenziellem TX gibt es ein geplantes Target–Referenz-Paar.""",
        ...
        "messages": {
            ...
            "result_success": "Nur Performance — eigenständige Target-Werte",
