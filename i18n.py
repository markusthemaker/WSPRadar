...
T = {
    ...
    "de": {
        ...
        "fig_drilldown_outlier_candidate": "Qualifizierende Kandidateneinheit",
...
RESULT_GUIDANCE = {
    "en": {
        ...
        "sections": {
            ...
            "drilldown_success_tx": {
                "read": """Drill-Down Data is the audit trail from TX Performance back to the contributing WSPR cycles. It shows <strong class="defined-term">processed retained evidence</strong> after WSPRadar’s eligibility rules and filters. The <strong class="defined-term">Target heard</strong> and <strong class="defined-term">Other signals heard</strong> columns identify counted outcomes; a row with neither count but with Target SNR is a Target report without the independent RX-activity confirmation required for the Decode Rate. Use exact UTC time, RX-station identity and these evidence columns to reconcile the numerator and denominator with the displayed summaries. With exactly one selected station, `Zoom window`, `Center date (UTC)` and `Center time (UTC)` define an exact centered interval. `Filter table` then changes only the displayed rows; it does not alter the focused plots or completed analysis. The focused SNR plot shows the actual normalized Target SNR of each successful confirmed opportunity at its canonical cycle time; unsuccessful opportunities have no SNR point. It applies no temporal median, IQR, density layer, full-run median or UTC-hour folding, while the companion outcome view retains its chronological evidence summary. Segment and full-window Selected Station Evidence remain aggregated density views.""",
    ...
    "de": {
        ...
        "sections": {
            ...
            "drilldown_success_tx": {
                "read": """Die Drill-Down-Daten sind der Prüfpfad vom TX-Performance-Ergebnis zurück zu den beitragenden WSPR-Zyklen. Sie zeigen <strong class="defined-term">verarbeitete beibehaltene Evidenz</strong> nach den Eignungsregeln und Filtern von WSPRadar. Die Spalten <strong class="defined-term">Target gehört</strong> und <strong class="defined-term">Andere Signale gehört</strong> kennzeichnen gezählte Outcomes; eine Zeile ohne einen der beiden Zählwerte, aber mit Target-SNR, ist ein Target-Report ohne die unabhängige Bestätigung der RX-Aktivität, die für die Dekodierrate erforderlich ist. Nutze exakte UTC-Zeit, RX-Stationsidentität und diese Evidenzspalten, um Zähler und Nenner mit den angezeigten Zusammenfassungen abzugleichen. Bei genau einer ausgewählten Station definieren `Zoom-Zeitfenster`, `Datum der Fenstermitte (UTC)` und `Uhrzeit der Fenstermitte (UTC)` ein exaktes zentriertes Intervall. `Tabelle filtern` verändert anschließend nur die angezeigten Zeilen, nicht die fokussierten Abbildungen oder die abgeschlossene Analyse. Die fokussierte SNR-Abbildung zeigt das tatsächliche normierte Target-SNR jeder erfolgreichen bestätigten Gelegenheit zu ihrer kanonischen Zykluszeit; erfolglose Gelegenheiten besitzen keinen SNR-Punkt. Zeitmedian, IQR, Dichteschicht, Median des vollständigen Laufs und Faltung nach UTC-Stunde entfallen, während die ergänzende Outcome-Ansicht ihre chronologische Evidenzzusammenfassung beibehält. Segmentansicht und Evidenz der ausgewählten Station über das vollständige Fenster bleiben aggregierte Dichteansichten.""",
