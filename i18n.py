...
T = {
    "en": {
        ...
        'fig_success_snr_chronological_subtitle_rx': "Each TX station centered on its run median · {time_bin} bins",
        'fig_success_snr_chronological_subtitle_tx': "Each RX station centered on its run median · {time_bin} bins",
        ...
        'fig_success_snr_utc_hour_subtitle_rx': "Each TX station centered on its run median · 1 h bins",
        'fig_success_snr_utc_hour_subtitle_tx': "Each RX station centered on its run median · 1 h bins",
        ...
        'fig_success_station_support_folded_subtitle': "Average contributing station presences per represented UTC date",
        'fig_success_opportunities_folded_subtitle': "Average confirmed opportunities per represented UTC date",
        ...
        'fig_success_selected_snr_chronological_subtitle': "Selected station · {time_bin} bins",
        'fig_success_selected_snr_utc_hour_title': "Successful Target SNR by UTC Hour",
        'fig_success_selected_snr_utc_hour_subtitle': "Selected station · 1 h bins",
        ...
        'fig_success_selected_station_support_folded_subtitle': "Average station presence per represented UTC date",
    ...
    "de": {
        ...
        'fig_success_snr_chronological_subtitle_rx': "Jede TX-Station auf ihren Laufmedian zentriert · {time_bin}-Bins",
        'fig_success_snr_chronological_subtitle_tx': "Jede RX-Station auf ihren Laufmedian zentriert · {time_bin}-Bins",
        ...
        'fig_success_snr_utc_hour_subtitle_rx': "Jede TX-Station auf ihren Laufmedian zentriert · 1-h-Bins",
        'fig_success_snr_utc_hour_subtitle_tx': "Jede RX-Station auf ihren Laufmedian zentriert · 1-h-Bins",
        ...
        'fig_success_station_support_folded_subtitle': "Durchschnittliche Stationspräsenzen pro berücksichtigtem UTC-Tag",
        'fig_success_opportunities_folded_subtitle': "Durchschnittliche bestätigte Gelegenheiten pro berücksichtigtem UTC-Tag",
        ...
        'fig_success_selected_snr_chronological_subtitle': "Ausgewählte Station · {time_bin}-Bins",
        'fig_success_selected_snr_utc_hour_title': "Erfolgreiches Target-SNR nach UTC-Stunde",
        'fig_success_selected_snr_utc_hour_subtitle': "Ausgewählte Station · 1-h-Bins",
        ...
        'fig_success_selected_station_support_folded_subtitle': "Durchschnittliche Stationspräsenz pro berücksichtigtem UTC-Tag",
    ...
    "en": {
        ...
        "sections": {
            ...
            "map_compare_rx": {
                ...
                "limits": """The map locates an observed pattern; it does not identify propagation mode, radiation angle, calibrated gain or physical cause. Its stepped dB color scale is symmetric around 0 dB and can expand between runs. It fits the visible sector range without fixed headroom. The light yellow-green display-neutral band's width matches the active step; at 3 dB it covers -1.5 to +1.5 dB. Only 0 dB means equality. Use the numerical color-bar values when comparing magnitudes across maps.""",
            },
            "map_compare_tx": {
                ...
                "limits": """The map does not identify propagation mode, radiation angle, actual radiated power or physical cause. An undecoded side has no SNR to compare or normalize. The stepped dB color scale is symmetric around 0 dB and can expand between runs. It fits the visible sector range without fixed headroom. The light yellow-green display-neutral band's width matches the active step; at 3 dB it covers -1.5 to +1.5 dB. Only 0 dB means equality. Use the numerical color-bar values when comparing magnitudes across maps.""",
            },
            "map_success_rx": {
            ...
            "success_temporal_evidence_tx": {
                ...
                "limits": """Folded station-bar height and folded Station-balanced Decode Rate use intentionally different weighting: the bar measures average daily participation, while the line weights each distinct receiver once. The SNR figure contains successful Target reports only and depends on reported transmit power. UTC-hour patterns show recurring associations, not their physical cause.""",
            },
            ...
            "selected_compare_joint": {
                "read": """Selected Station Evidence isolates one chosen {peer_type} station and one specific radio path. **Δ SNR over Time** uses the selected chronological bin; **Δ SNR by UTC Hour** folds the path into 1-hour slots. Values are signed and not baseline-normalized: positive favors Target, negative favors Reference and 0 dB is equality. Red dashed line: median of all Joint Spots; pale markers: bin medians. Color shows relative Joint-Spot density per panel, not advantage; height gives labelled dB on the nonlinear axis. Gaps mean no Joint evidence. <strong class="defined-term">Selected Path Evidence Coverage</strong> adds Only Target, Joint and Only Reference. Bars count **Retained WSPR Cycles**; the amber line shows their Joint share. Folded bars average per represented UTC date. A stable sign, dense evidence and high Joint share support a persistent path difference; reversals or long one-sided periods indicate dependence on time or conditions.""",
                "limits": """This is one observation-weighted path, not the station-balanced segment result. Simultaneous Compare retains only cycles with observed Target activity. RX: Target decoded a qualifying signal; TX: Target was decoded somewhere. Reference has no equivalent gate. Target-offline cycles are excluded; missing Reference may become Only Target. One-sided coverage is therefore Target-favoring and shows availability, not symmetric wins and losses. Joint ΔSNR is unaffected because Joint Spots contain both sides. UTC recurrence does not establish cause.""",
            },
            "selected_compare_scheduled": {
                "read": """Selected Station Evidence isolates one RX station and one scheduled radio path. **Δ SNR over Time** uses the selected chronological bin; **Δ SNR by UTC Hour** folds the path into 1-hour slots. Values are signed and not baseline-normalized: positive favors Target, negative favors Reference and 0 dB is equality. The red dashed line is the median of complete Scheduled Pairs; pale markers are bin medians. Color shows relative Scheduled-Pair density per panel, not advantage; height gives labelled dB on the nonlinear axis. Gaps mean no complete pair was available. <strong class="defined-term">Selected Path Evidence Coverage</strong> adds Only Target, Joint and Only Reference. Bars count **Scheduled A/B Pairs**; the amber line shows the complete-pair share. Folded bars average per represented UTC date. A stable sign, dense evidence and high completed-pair share support a persistent path difference; reversals or long one-sided periods indicate dependence on time or conditions.""",
                "limits": """This is one observation-weighted receiver path, not the station-balanced segment result. Scheduled Compare follows the configured planned Target–Reference pairs. Pair ΔSNR exists only when both sides were decoded. One-sided units show incomplete-pair coverage, contain no missing-side SNR and are not symmetric wins or losses. Time separation allows propagation, QRM, switching and schedule phase to resemble a path difference. UTC recurrence does not establish cause.""",
            },
            "selected_success_rx": {
                "read": """Selected Station Evidence isolates one remote TX station from the active RX Performance scope. Selecting another Station Insights row replaces the current station. The upper <strong class="defined-term">Selected Station SNR Evidence</strong> figure uses the same chronological and folded UTC-hour layout as Temporal Evidence, but shows this path’s actual normalized successful Target SNR rather than deviation from a station baseline. Chronological color shows where successful SNR observations concentrate in each selected time bin, and the line shows their bin median. The UTC-hour panel first gives every represented date one hourly median, then shows how those values recur across dates. The lower <strong class="defined-term">Selected Station Temporal Evidence</strong> figure separates station presence from opportunity depth. When the selected TX station contributes evidence in a chronological bin, it supplies one split station vote between Heard by Target and Heard by others only. The folded station row shows its average presence per represented UTC date. The opportunity row counts every confirmed opportunity chronologically and shows average counts per represented date after UTC-hour folding. Green shares and Decode Rate lines show the same selected-path rate. Because exactly one station is selected, station-balanced and Opportunity-level Decode Rates are identical; matching lines are expected. Read SNR and Decode Rate together: strong successful decodes with a falling rate can mean weaker signals were no longer decoded.""",
                "limits": """The SNR figure contains successful Target decodes only; missed signals have no recorded Target SNR. Folded UTC-hour patterns show recurring associations, not whether propagation, local noise, interference or equipment changes caused them. SNR normalization depends on reported transmit power and the selected callsign-plus-locator identity.""",
            },
            "selected_success_tx": {
                "read": """Selected Station Evidence isolates one remote RX station from the active TX Performance scope. Selecting another Station Insights row replaces the current receiver. The upper <strong class="defined-term">Selected Station SNR Evidence</strong> figure uses the same chronological and folded UTC-hour layout as Temporal Evidence, but shows the actual normalized Target SNR reported by this receiver rather than deviation from a station baseline. Chronological color shows where successful Target reports concentrate in each selected time bin, and the line shows their bin median. The UTC-hour panel first gives every represented date one hourly median, then shows how those values recur across dates. The lower <strong class="defined-term">Selected Station Temporal Evidence</strong> figure separates receiver presence from opportunity depth. When the selected RX station contributes evidence in a chronological bin, it supplies one split station vote between Target heard and Other signals heard only. The folded station row shows its average presence per represented UTC date. The opportunity row counts every confirmed opportunity chronologically and shows average counts per represented date after UTC-hour folding. Green shares and Decode Rate lines show the same selected-path rate. Because exactly one receiver is selected, station-balanced and Opportunity-level Decode Rates are identical; matching lines are expected. Read SNR and Decode Rate together: strong successful reports with a falling rate can mean weaker Target signals were no longer decoded.""",
                "limits": """The SNR figure contains successful Target reports only; a missed Target has no reported SNR. Folded UTC-hour patterns show recurring associations, not whether propagation, receiver noise, interference or Target changes caused them. Normalization depends on the reported Target power.""",
            },
    ...
    "de": {
        ...
        "sections": {
            ...
            "map_compare_rx": {
                ...
                "limits": """Die Karte lokalisiert ein beobachtetes Muster; sie bestimmt weder Ausbreitungsart, Abstrahlwinkel, kalibrierten Gewinn noch physische Ursache. Ihre abgestufte dB-Farbskala ist symmetrisch um 0 dB und kann sich zwischen Läufen erweitern. Sie passt sich ohne feste Reserve an den sichtbaren Segmentwertebereich an. Die Breite des hellen gelbgrünen darstellungsneutralen Bands entspricht der aktiven Schrittweite; bei 3 dB reicht es von -1,5 bis +1,5 dB. Nur 0 dB bedeutet Gleichheit. Vergleiche Größen über mehrere Karten anhand der numerischen Werte der Farbskala.""",
            },
            "map_compare_tx": {
            ...
            "temporal_evidence_joint": {
                "read": """Zeitliche Evidenz verbindet ΔSNR und Evidenzbasis. **Δ SNR im Zeitverlauf** nutzt gewählte Binbreite; **Δ SNR nach UTC-Stunde** faltet Tage in 1-h-Slots. Positiv spricht für Target, negativ für Referenz; 0 dB bedeutet Gleichheit. Rote gestrichelte Linie: Median der Joint Spots; helle Marker: Bin-Mediane. Farbe: relative Joint-Spot-Dichte pro Panel, nicht Vorteil; Höhe: dB-Wert auf nichtlinearer Achse. <strong class="defined-term">Zeitliche Evidenzabdeckung</strong> zeigt Only Target, Joint und Only Reference. Oben liefert jede Station eine aufgeteilte Stimme; die Balkenhöhe zeigt beitragende Stationen, die blaue Linie deren gleichgewichteten Joint-Anteil. Unten: Vergleichseinheiten; gelbe Linie: Joint-Anteil der Einheiten. Linienabstand zeigt Volumengewichtung. Gefaltete Balken mitteln je UTC-Stunde über berücksichtigte Tage. Stabiles Vorzeichen, dichte Evidenz und ähnliche Linien stützen Wiederkehr; Spitzen, Wechsel oder geringer Joint-Anteil mahnen zur Vorsicht.""",
                "limits": """Simultaner Compare nutzt nur Zyklen mit beobachteter Target-Aktivität: RX: Target decodierte ein qualifizierendes Signal; TX: Target wurde irgendwo decodiert. Es gibt kein entsprechendes Referenz-Gate. Target-Offline-Zyklen entfallen; fehlende Referenz-Evidenz kann Only Target werden. Einseitige Abdeckung ist daher Target-begünstigt und zeigt Verfügbarkeit, keine symmetrischen Siege oder Niederlagen. Joint-ΔSNR bleibt unverändert, da Joint Spots beide Seiten enthalten. UTC-Wiederkehr belegt keine Ursache.""",
            },
            "temporal_evidence_scheduled": {
                "read": """Zeitliche Evidenz verbindet Pair-ΔSNR und Evidenzbasis. **Δ SNR im Zeitverlauf** nutzt gewählte Binbreite; **Δ SNR nach UTC-Stunde** faltet Tage in 1-h-Slots. Positive Werte sprechen für Target, negative für Referenz; 0 dB bedeutet Gleichheit. Rote gestrichelte Linie: Median vollständiger geplanter Paare; helle Marker: Bin-Mediane. Farbe: relative Dichte geplanter Paare je Panel, nicht Vorteil; Höhe: dB-Wert auf nichtlinearer Achse. <strong class="defined-term">Zeitliche Evidenzabdeckung</strong> zeigt Only Target, Joint und Only Reference. Oben liefert jede RX-Station eine aufgeteilte Stimme; die Balkenhöhe zeigt beitragende Empfänger, die blaue Linie deren gleichgewichteten Anteil vollständiger Paare. Unten: geplante A/B-Einheiten; gelbe Linie: Anteil vollständiger Paare. Linienabstand zeigt Volumengewichtung. Gefaltete Balken mitteln je UTC-Stunde über vertretene Tage. Stabiles Vorzeichen, dichte Evidenz und ähnliche Linien stützen Wiederkehr; Spitzen, Wechsel oder geringer Paaranteil mahnen zur Vorsicht.""",
                "limits": """Scheduled Compare folgt den konfigurierten Target–Referenz-Paaren. Pair-ΔSNR entsteht nur, wenn beide geplanten Seiten decodiert wurden. Only Target und Only Reference zeigen unvollständige Paare, ohne SNR der anderen Seite; sie sind keine symmetrischen Siege oder Niederlagen. Der Zeitabstand lässt Ausbreitung, QRM, Umschaltung und Zeitplanphase in Pair-ΔSNR einfließen. UTC-Wiederkehr belegt keine Ursache.""",
            },
            "success_evidence_rx": {
