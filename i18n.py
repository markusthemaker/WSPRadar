...
T = {
    "en": {
        ...
        "sub_results_success_evidence": "At-least-once reach, Decode Rate and successful-signal strength by calculated distance for the active scope.",
        "sub_results_success_temporal": "Successful signal-strength deviations, station support, confirmed-opportunity volume and both Decode Rate weightings shown chronologically and by UTC hour.",
        ...
        "fmt_success_selected_context": "{station} ({locator}) · {distance_km} km · {azimuth_degrees}° {direction} · {confirmed_opportunities} {opportunity_unit} · Decode Rate {success_rate}% · Median successful Target SNR {median_snr}",
        "abbr_compass_east": "E",
        ...
        "fig_compare_delta_distribution": "\u0394 SNR Distribution",
        "fig_mean_label": "Mean",
        ...
        "fig_no_data": "No data",
        "fig_selected_evidence_empty": "No selected evidence available.",
        "fig_single_evidence_point": "single evidence point",
        "fig_single_point_title": "{title} \u2014 single evidence point",
        "msg_loading": "\u23f3 Loading...",
    ...
    "de": {
        ...
        "sub_results_success_evidence": "Mindestens-einmal-Reichweite, Dekodierrate und Stärke erfolgreicher Signale nach berechneter Entfernung im aktiven Bereich.",
        "sub_results_success_temporal": "Abweichungen erfolgreicher Signalstärken, Stationsstützung, Volumen bestätigter Gelegenheiten und beide Gewichtungen der Dekodierrate, chronologisch und nach UTC-Stunde dargestellt.",
        ...
        "sub_results_selected_station_single": "{station} ({locator}) · {evidence_count} {evidence_unit}",
        "fmt_success_selected_context": "{station} ({locator}) · {distance_km} km · {azimuth_degrees}° {direction} · {confirmed_opportunities} {opportunity_unit} · Dekodierrate {success_rate}% · Median des erfolgreichen Target-SNR {median_snr}",
        "abbr_compass_east": "O",
        ...
        "fig_compare_delta_distribution": "\u0394 SNR Verteilung",
        ...
        "fig_selected_evidence_empty": "Keine Evidenz f\u00fcr die Auswahl verf\u00fcgbar.",
        "fig_single_evidence_point": "einzelner Evidenzpunkt",
        "fig_single_point_title": "{title} \u2014 einzelner Evidenzpunkt",
...
RESULT_GUIDANCE = {
    "en": {
        ...
        "sections": {
            ...
            "station_insights_success_tx": {
                "read": """Station Insights identifies the RX stations behind the scoped TX Performance result. Each row represents one reported callsign-plus-locator identity. Read <strong class="defined-term">Target heard</strong>, <strong class="defined-term">Other signals heard only</strong>, confirmed opportunities and Decode Rate together. A high rate based on many opportunities carries more descriptive depth than the same rate based on only a few. Median successful Target SNR describes only successful Target reports; a less-negative value is stronger relative to noise. Table filters, sorting and row selection change only the displayed station view and do not recalculate the Segment Inspector figures.""",
            ...
            "selected_compare_joint": {
                "read": """Selected Station Evidence zooms from the station-balanced segment result into one chosen {peer_type} station. Every Joint Spot in the view therefore belongs to <strong class="defined-term">one specific radio path</strong>. **Δ SNR Distribution** shows the center, spread and outliers of that station's paired observations; read `Share (%)` together with the numerical median and mean. **Chronological** places those same observations in their real UTC order, while **UTC-Hour** folds the represented dates onto one 24-hour clock. Selecting another Station Insights row replaces the current station. Use this view to check whether the scoped segment result is also supported on this path and remains stable through time; repeated agreement across many paired observations and time bins is stronger evidence than one isolated cluster.""",
                "limits": """This is an observation-weighted view of one selected path and does not replace the segment's station-balanced summary. A time pattern still does not identify its physical cause.""",
            },
            "selected_compare_scheduled": {
                "read": """Selected Station Evidence zooms from the station-balanced segment result into one chosen RX station. Every Scheduled Pair in the view therefore belongs to <strong class="defined-term">one specific radio path</strong>. **Δ SNR Distribution** shows the center, spread and outliers of Pair ΔSNR; read the numerical median and mean together. **Chronological** places those same pairs in their planned UTC sequence, while **UTC-Hour** folds the represented dates onto one 24-hour clock. Selecting another Station Insights row replaces the current receiver. Use this view to see whether the scheduled result recurs on this path and through time; agreement across many pairs and time bins is stronger than one isolated cluster.""",
                "limits": """This is an observation-weighted view of one selected path and does not replace the segment's station-balanced summary. It cannot separate a path difference from propagation, switching or schedule-phase effects.""",
            },
            "selected_success_rx": {
            ...
            "drilldown_success_tx": {
                "read": """Drill-Down Data is the audit trail from TX Performance back to the contributing WSPR cycles. It shows processed evidence after WSPRadar’s eligibility rules and filters. Outcomes identify <strong class="defined-term">Target heard</strong>, <strong class="defined-term">Other signals heard only</strong>, and Target reports without the independent RX-activity confirmation required for the Decode Rate. Use exact UTC time, RX-station identity and outcome to reconcile the numerator and denominator with the displayed summaries. Table filters change only the visible rows.""",
    ...
    "de": {
        ...
        "sections": {
            ...
            "station_insights_compare_joint": {
                "read": """Station Insights nennt die {peer_type}-Stationen hinter dem gepaarten Ergebnis. Jede Zeile ist eine im Archiv durch Rufzeichen und Locator definierte Identität. Lies <strong class="defined-term">Joint Spots</strong> als Evidenztiefe, die einseitigen Anzahlen als Kontext der Decode Outcomes und das mediane Stations-ΔSNR als typischen gepaarten Unterschied dieser Station. Sortieren nach Joint Spots hilft, ein wiederholbares Stationsergebnis von einem Extremwert mit wenig Evidenz zu trennen; der Vergleich vieler Zeilen zeigt, ob das Kartenmuster breit getragen oder auf wenige Stationen konzentriert ist. `Ungepaarte Evidenz einbeziehen` ergänzt Stationen mit einseitiger oder asynchroner Evidenz ohne nutzbares ΔSNR. Wähle eine Zeile für die detaillierte Evidenz dieser Station; eine andere Zeile ersetzt die bisherige Auswahl. Tabellenfilter ändern nur Anzeige und Auswahl.""",
            ...
            "station_insights_compare_scheduled": {
                "read": """Station Insights nennt die RX-Stationen hinter dem geplanten Ergebnis. Jede Zeile ist eine im Archiv durch Rufzeichen und Locator definierte Identität. Lies <strong class="defined-term">Scheduled Pairs</strong> als Evidenztiefe, die einseitigen geplanten Anzahlen als Kontext der Decode Outcomes und das mediane Paar-ΔSNR als typischen geplanten Unterschied dieser Station. Sortieren nach Paaranzahl hilft, ein wiederholbares Stationsergebnis von einem Extremwert mit wenigen Paaren zu trennen; der Vergleich vieler Zeilen zeigt, ob das Muster breit getragen oder auf wenige Empfänger konzentriert ist. `Ungepaarte Evidenz einbeziehen` ergänzt Stationen ohne vollständiges Paar. Wähle eine Zeile für die detaillierte Evidenz dieses Empfängers; eine andere Zeile ersetzt die bisherige Auswahl. Tabellenfilter ändern nur Anzeige und Auswahl.""",
            ...
            "selected_compare_joint": {
                "read": """Die Evidenz der ausgewählten Station zoomt vom stationsgleichgewichteten Segmentergebnis in eine gewählte {peer_type}-Station. Jeder Joint Spot in dieser Ansicht gehört damit zu <strong class="defined-term">genau einem Funkweg</strong>. **Δ SNR Verteilung** zeigt Zentrum, Streuung und Ausreißer der gepaarten Beobachtungen dieser Station; lies `Anteil (%)` zusammen mit numerischem Median und Mittelwert. **Chronologisch** ordnet dieselben Beobachtungen in ihrer tatsächlichen UTC-Folge an, während **UTC-Stunde** die dargestellten Tage auf eine gemeinsame 24-Stunden-Uhr legt. Eine andere Zeile in Station Insights ersetzt die bisherige Station. Nutze diese Ansicht, um zu prüfen, ob das Segmentergebnis auch auf diesem Funkweg gestützt wird und zeitlich stabil bleibt; wiederholte Übereinstimmung über viele gepaarte Beobachtungen und Zeit-Bins ist stärkere Evidenz als eine isolierte Häufung.""",
                "limits": """Das ist eine beobachtungsgewichtete Ansicht eines ausgewählten Funkwegs und ersetzt nicht die stationsgleichgewichtete Segmentzusammenfassung. Ein Zeitmuster benennt weiterhin keine physische Ursache.""",
            },
            "selected_compare_scheduled": {
                "read": """Die Evidenz der ausgewählten Station zoomt vom stationsgleichgewichteten Segmentergebnis in eine gewählte RX-Station. Jedes Scheduled Pair in dieser Ansicht gehört damit zu <strong class="defined-term">genau einem Funkweg</strong>. **Δ SNR Verteilung** zeigt Zentrum, Streuung und Ausreißer des Paar-ΔSNR; lies numerischen Median und Mittelwert gemeinsam. **Chronologisch** ordnet dieselben Paare in ihrer geplanten UTC-Folge an, während **UTC-Stunde** die dargestellten Tage auf eine gemeinsame 24-Stunden-Uhr legt. Eine andere Zeile in Station Insights ersetzt den bisherigen Empfänger. Nutze diese Ansicht, um zu prüfen, ob sich das geplante Ergebnis auf diesem Funkweg und über die Zeit wiederholt; Übereinstimmung über viele Paare und Zeit-Bins ist stärker als eine isolierte Häufung.""",
                "limits": """Das ist eine beobachtungsgewichtete Ansicht eines ausgewählten Funkwegs und ersetzt nicht die stationsgleichgewichtete Segmentzusammenfassung. Sie kann einen Pfadunterschied nicht von Ausbreitungs-, Schalt- oder Zeitplanphaseneffekten trennen.""",
            },
            "selected_success_rx": {
...
def absolute_terms(t, mode):
    ...
    return {
        ...
        "presentation_subtext": t[f"success_{mode_key}_subtext"],
