...
RESULT_GUIDANCE = {
    "en": {
        ...
        "sections": {
            ...
            "temporal_evidence_joint": {
                "read": """Temporal Evidence links ΔSNR to its support. **Δ SNR over Time** uses the selected chronological bin; **Δ SNR by UTC Hour** folds dates into 1-hour slots. Positive values favor Target; negative values favor Reference; 0 dB is equality. Red dashed line: median of all Joint Spots; pale markers: bin medians. A subtle IQR band bounded by fine Q1–Q3 lines shows the middle 50% of that same bin population only when at least five Joint Spots contribute. Color shows relative Joint-Spot density per panel, not |ΔSNR| or advantage; height gives labelled dB on the nonlinear axis. <strong class="defined-term">Benchmark Temporal Evidence Coverage</strong> adds Only Target, Joint and Only Reference. Upper row: each station supplies one split vote; bar height shows contributing stations, and the blue line shows their equal-weight Joint share. Lower row: comparison-unit counts; the amber line shows Joint divided by retained units. A gap between lines shows volume weighting. Folded bars average each UTC hour over represented dates. Stable same-sign medians, dense evidence and similar lines support recurrence; spikes, reversals or low Joint share deserve caution.""",
            ...
            "temporal_evidence_scheduled": {
                "read": """Temporal Evidence links Pair ΔSNR to completed-pair support. **Δ SNR over Time** uses the selected chronological bin; **Δ SNR by UTC Hour** folds dates into 1-hour slots. Positive values favor Target; negative values favor Reference; 0 dB is equality. Red dashed line: median of complete Scheduled Pairs; pale markers: bin medians. A subtle IQR band bounded by fine Q1–Q3 lines shows the middle 50% of that same bin population only when at least five complete pairs contribute. Color shows relative Scheduled-Pair density per panel, not advantage; height gives labelled dB on the nonlinear axis. <strong class="defined-term">Benchmark Temporal Evidence Coverage</strong> adds Only Target, Joint and Only Reference. Upper row: each receiver supplies one split vote; bar height shows contributing receivers, and the blue line shows their equal-weight completed-pair share. Lower row: scheduled A/B-unit counts; the amber line shows complete pairs divided by retained units. A gap between lines shows volume weighting. Folded bars average each UTC hour over represented dates. Stable same-sign medians, dense evidence and similar lines support recurrence; spikes, reversals or low completed-pair share deserve caution.""",
    ...
    "de": {
        ...
        "sections": {
            ...
            "temporal_evidence_scheduled": {
                "read": """Zeitliche Evidenz verbindet Pair-ΔSNR und Evidenzbasis. **Δ SNR im Zeitverlauf** nutzt gewählte Binbreite; **Δ SNR nach UTC-Stunde** faltet Tage in 1-h-Slots. Positive Werte sprechen für Target, negative für Referenz; 0 dB bedeutet Gleichheit. Rote gestrichelte Linie: Median vollständiger geplanter Paare; helle Marker: Bin-Mediane. Ein dezentes IQR-Band, begrenzt von feinen Q1–Q3-Linien, zeigt die mittleren 50 % derselben Bin-Population nur, wenn mindestens fünf vollständige Paare beitragen. Farbe: relative Dichte geplanter Paare je Panel, nicht Vorteil; Höhe: dB-Wert auf nichtlinearer Achse. <strong class="defined-term">Zeitliche Benchmark-Evidenzabdeckung</strong> zeigt Only Target, Joint und Only Reference. Oben liefert jede RX-Station eine aufgeteilte Stimme; die Balkenhöhe zeigt beitragende Empfänger, die blaue Linie deren gleichgewichteten Anteil vollständiger Paare. Unten: geplante A/B-Einheiten; gelbe Linie: Anteil vollständiger Paare. Linienabstand zeigt Volumengewichtung. Gefaltete Balken mitteln je UTC-Stunde über vertretene Tage. Stabiles Vorzeichen, dichte Evidenz und ähnliche Linien stützen Wiederkehr; Spitzen, Wechsel oder geringer Paaranteil mahnen zur Vorsicht.""",
