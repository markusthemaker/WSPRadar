...
T = {
    "en": {
        ...
        'fig_selected_compare_chronological_subtitle': "Selected station \u00b7 {time_bin} bins",
        'fig_selected_compare_folded_subtitle': "Selected station \u00b7 1 h bins",
    ...
    "de": {
        ...
        'fig_selected_compare_chronological_subtitle': "Ausgew\u00e4hlte Station \u00b7 {time_bin}-Bins",
        'fig_selected_compare_folded_subtitle': "Ausgew\u00e4hlte Station \u00b7 1-h-Bins",
...
RESULT_GUIDANCE = {
    "en": {
        ...
        "sections": {
            ...
            "selected_compare_joint": {
                "read": """Selected Station Evidence zooms from the station-balanced segment result into one chosen {peer_type} station. Every Joint Spot in the view therefore belongs to <strong class="defined-term">one specific radio path</strong>. **Δ SNR over Time** places those paired observations in their real UTC sequence using the selected aggregation bin. **Δ SNR by UTC Hour** simultaneously folds the same observation-level rows from all represented dates into fixed one-hour slots on a 24-hour UTC clock. Both panels share the selected path's presentation-only, median-centered nonlinear Δ SNR scale; the axis labels remain absolute dB values. Color is relative density within each panel, and the connected markers show the median of each populated bin. Selecting another Station Insights row replaces the current station. Use repeated agreement across many Joint Spots and several time bins—not one isolated cluster—as the practical reading check.""",
                "limits": """This is an observation-weighted view of one selected path and does not replace the segment's station-balanced summary. UTC-hour folding pools qualifying rows rather than giving every represented date equal weight, and a recurring time pattern does not identify its physical cause.""",
            },
            "selected_compare_scheduled": {
                "read": """Selected Station Evidence zooms from the station-balanced segment result into one chosen RX station. Every Scheduled Pair in the view therefore belongs to <strong class="defined-term">one specific radio path</strong>. **Δ SNR over Time** places those pairs in their planned UTC sequence using the selected aggregation bin. **Δ SNR by UTC Hour** simultaneously folds the same observation-level pairs from all represented dates into fixed one-hour slots on a 24-hour UTC clock. Both panels share the selected path's presentation-only, median-centered nonlinear Pair Δ SNR scale; the axis labels remain absolute dB values. Color is relative density within each panel, and the connected markers show the median of each populated bin. Selecting another Station Insights row replaces the current receiver. Look for agreement across many Scheduled Pairs and several time bins rather than relying on one isolated cluster.""",
                "limits": """This is an observation-weighted view of one selected path and does not replace the segment's station-balanced summary. UTC-hour folding pools qualifying pairs rather than giving every represented date equal weight, and it cannot separate a path difference from propagation, switching or schedule-phase effects.""",
            },
            "selected_success_rx": {
    ...
    "de": {
        ...
        "sections": {
            ...
            "selected_compare_joint": {
                "read": """Die Evidenz der ausgewählten Station zoomt vom stationsgleichgewichteten Segmentergebnis in eine gewählte {peer_type}-Station. Jeder Joint Spot in dieser Ansicht gehört damit zu <strong class="defined-term">genau einem Funkweg</strong>. **Δ SNR im Zeitverlauf** ordnet diese gepaarten Beobachtungen mit der gewählten Aggregationsbreite in ihrer tatsächlichen UTC-Folge an. **Δ SNR nach UTC-Stunde** faltet gleichzeitig dieselben Evidenzzeilen auf Beobachtungsebene aus allen berücksichtigten Tagen in feste einstündige Klassen einer 24-Stunden-UTC-Uhr. Beide Panels verwenden dieselbe ausschließlich für die Darstellung medianzentrierte, nichtlineare Δ-SNR-Skala des ausgewählten Funkwegs; die Achsenbeschriftungen bleiben absolute dB-Werte. Die Farbe zeigt die relative Dichte innerhalb des jeweiligen Panels, die verbundenen Marker den Median jedes belegten Bins. Eine andere Zeile in Station Insights ersetzt die bisherige Station. Prüfe praktisch die wiederholte Übereinstimmung über viele Joint Spots und mehrere Zeit-Bins, statt dich auf eine isolierte Häufung zu stützen.""",
                "limits": """Das ist eine beobachtungsgewichtete Ansicht eines ausgewählten Funkwegs und ersetzt nicht die stationsgleichgewichtete Segmentzusammenfassung. Die Faltung nach UTC-Stunde bündelt qualifizierende Zeilen, statt jeden berücksichtigten Tag gleich zu gewichten; ein wiederkehrendes Zeitmuster benennt keine physische Ursache.""",
            },
            "selected_compare_scheduled": {
                "read": """Die Evidenz der ausgewählten Station zoomt vom stationsgleichgewichteten Segmentergebnis in eine gewählte RX-Station. Jedes Scheduled Pair in dieser Ansicht gehört damit zu <strong class="defined-term">genau einem Funkweg</strong>. **Δ SNR im Zeitverlauf** ordnet diese Paare mit der gewählten Aggregationsbreite in ihrer geplanten UTC-Folge an. **Δ SNR nach UTC-Stunde** faltet gleichzeitig dieselben Paare auf Beobachtungsebene aus allen berücksichtigten Tagen in feste einstündige Klassen einer 24-Stunden-UTC-Uhr. Beide Panels verwenden dieselbe ausschließlich für die Darstellung medianzentrierte, nichtlineare Paar-Δ-SNR-Skala des ausgewählten Funkwegs; die Achsenbeschriftungen bleiben absolute dB-Werte. Die Farbe zeigt die relative Dichte innerhalb des jeweiligen Panels, die verbundenen Marker den Median jedes belegten Bins. Eine andere Zeile in Station Insights ersetzt den bisherigen Empfänger. Achte auf Übereinstimmung über viele Scheduled Pairs und mehrere Zeit-Bins, statt eine isolierte Häufung überzubewerten.""",
                "limits": """Das ist eine beobachtungsgewichtete Ansicht eines ausgewählten Funkwegs und ersetzt nicht die stationsgleichgewichtete Segmentzusammenfassung. Die Faltung nach UTC-Stunde bündelt qualifizierende Paare, statt jeden berücksichtigten Tag gleich zu gewichten; sie kann einen Pfadunterschied nicht von Ausbreitungs-, Schalt- oder Zeitplanphaseneffekten trennen.""",
            },
            "selected_success_rx": {
